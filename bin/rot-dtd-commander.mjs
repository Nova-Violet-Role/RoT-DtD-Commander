...
function produce(t) {
  if (t.main) {
    ...
    let text = p.resolved.text;
    // TEI-stamp discipline: a resolved file names the source it was
    // generated from, so a hand edit lands on the source or not at all.
    // build --check already refuses drift; the stamp tells the reader where
    // the fix goes. After the front matter, never inside it; no double
    // hyphen in the stamp text, per rule C12.
    const rel = relative(ROOT, t.src).split(sep).join('/');
    const stamp = `<!-- Generated by rdc build from ${rel}: edit the source, never this file. -->`;
    const fm = /^---\r?\n[\s\S]*?\r?\n---\r?\n/.exec(text);
    text = fm ? text.slice(0, fm[0].length) + stamp + '\n' + text.slice(fm[0].length) : stamp + '\n' + text;
    return { text, report: p.report, binary: null };
  }
