...
export function render({ sigils, commands, skills, agents }) {
  ...
  out.push(BEGIN);
  out.push('<details>');
  out.push(`<summary><b>Open the full index</b> &mdash; ${commands.length} commands in ${fams.length} families, ${skills.length} skills, ${agents.length} agents</summary>`);
  out.push('');
  ...
  out.push(`  R["RoT DtD Commander<br/>${commands.length} commands"]`);
  for (const f of fams) out.push(`  R --> ${f.id}["${f.sigil} ${f.name} · ${f.rows.length}"]`);
  out.push('```');
  out.push('');
...
function main() {
  ...
  console.log(`readme-index: ${summary}; README block written`);
}
