...
export function renderSvg(entries, theme) {
  ...
  const rowH = 26, top = 74, padL = 250, barW = 470;
  ...
  S.push(`<text x="24" y="34" font-size="17" font-weight="600" fill="${fg}">The Suite, by family</text>`);
  S.push(`<text x="24" y="55" font-size="12" fill="${dim}">${entries.length} entries. Every count measured from the resolved tree.</text>`);
  all.forEach((r, i) => {
    const y = top + i * rowH;
    const w = Math.max(2, Math.round((r.n / max) * barW));
    S.push(`<text x="${padL - 10}" y="${y + 12}" font-size="12" fill="${fg}" text-anchor="end">${esc(r.name)}</text>`);
    S.push(`<rect x="${padL}" y="${y + 2}" width="${w}" height="14" rx="3" fill="${r.color}"/>`);
    S.push(`<text x="${padL + w + 8}" y="${y + 13}" font-size="11" fill="${dim}">${r.n}</text>`);
  });
...
export function renderFamilySvg(entries, famName, theme) {
  ...
  const W = Math.max(900, Math.min(1500, Math.round(Math.max(longestCall * 7.4, longestDesc * 6.5) + 90)));
  const rowH = 40, top = 78;
  ...
  S.push(`<text x="26" y="34" font-size="16" font-weight="600" fill="${fg}">${esc(famName)}</text>`);
  S.push(`<text x="${W - 26}" y="34" font-size="13" fill="${dim}" text-anchor="end">${rows.length}</text>`);
  const meta = famMeta(famName);
  if (meta && meta.note) S.push(`<text x="26" y="56" font-size="11.5" fill="${dim}">${esc(clip(meta.note, 160))}</text>`);
  S.push(`<line x1="26" y1="${top - 14}" x2="${W - 26}" y2="${top - 14}" stroke="${line}" stroke-width="1"/>`);
  rows.forEach((e, i) => {
    const y = top + i * rowH;
    const call = callOf(e);
    const w = Math.round(7.35 * call.length + 14);
    S.push(`<rect x="26" y="${y - 12}" width="${w}" height="19" rx="4" fill="${chip}"/>`);
    S.push(`<text x="33" y="${y + 2}" font-size="12" font-family="ui-monospace,SFMono-Regular,Menlo,Consolas,monospace" fill="${fg}">${esc(call)}</text>`);
    S.push(`<text x="33" y="${y + 20}" font-size="11.5" fill="${dim}">${esc(e.description)}</text>`);
    if (e.laws) S.push(`<text x="${W - 26}" y="${y + 2}" font-size="10.5" fill="${dim}" text-anchor="end">${e.laws} laws</text>`);
  });
  S.push('</svg>');
  return S.join(NL) + NL;
}
...
export function renderReadme(entries) {
  ...
  L.push('</picture>');
  L.push('');
  L.push('Open a family for its plate: every entry with the exact call and what it does.');
  L.push('The names complete themselves as you type them in the agent, so the plate is the');
  L.push('reference and the terminal is the copy surface.');
  for (const f of order) {
    const meta = famMeta(f);
    L.push('');
    L.push('<details>');
    L.push('<summary><b>' + f + '</b> &middot; ' + byFam.get(f).length + '</summary>');
    L.push('');
    L.push('<picture>');
    L.push('  <source media="(prefers-color-scheme: dark)" srcset="docs/family-' + slug(f) + '-dark.svg" />');
    L.push('  <img alt="' + f + '" src="docs/family-' + slug(f) + '.svg" />');
    L.push('</picture>');
    L.push('');
    L.push('</details>');
  }
...
export function controls() {
  ...
  bad += say(rm.indexOf('<details>') !== -1 && rm.indexOf('| what you type |') === -1, 'every family folds, and the plate replaced the table rather than sitting beside it');
