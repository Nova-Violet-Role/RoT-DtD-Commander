...
export function renderReadme(entries) {
  ...
  L.push('</picture>');
  L.push('');
  L.push('Every command, skill and agent below carries the exact call you type. Use your');
  L.push('browser find; nothing here is folded.');
  for (const f of order) {
    const meta = famMeta(f);
    L.push('');
    L.push('#### ' + f + '  ' + '');
    if (meta && meta.note) { L.push(''); L.push(meta.note); }
    L.push('');
    L.push('| what you type | what it does | laws |');
    L.push('|---|---|---|');
    for (const e of byFam.get(f)) {
      const call = e.kind === 'command'
        ? (e.hint ? e.name + ' ' + e.hint : e.name)
        : e.kind === 'skill' ? e.name + ' (loads itself)' : e.name + ' (subagent)';
      const cell = (s) => String(s).split('|').join('\\|');
      L.push('| ' + BTQ + cell(call) + BTQ + ' | ' + cell(e.description) + ' | ' + (e.laws || '') + ' |');
    }
  }
  L.push('');
  L.push(RM_END);
...
export function controls() {
  ...
  bad += say(rm.indexOf('<details') === -1, 'trip: the README block folds nothing; a reference behind a click is the problem it was built to fix');
...
function main(argv) {
  ...
  if (argv.includes('--check')) {
    ...
    for (const [p, want, label] of [[SVG_LIGHT, renderSvg(entries, 'light'), 'docs/glossary-map.svg'], [SVG_DARK, renderSvg(entries, 'dark'), 'docs/glossary-map-dark.svg']]) {
