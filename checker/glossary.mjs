...
export function controls() {
  ...
  console.log('glossary controls: 10 run, ' + bad + ' failing');
  return bad;
}
...
function main(argv) {
  ...
  if (argv.includes('--check')) {
    ...
    console.log('glossary: ' + summary + '; docs/glossary.xhtml in step');
    process.exit(0);
  }

  write(text);
  const unfiled = entries.filter((e) => familyOf(e) === 'Unfiled').length;
  console.log('glossary: ' + summary + '; docs/glossary.xhtml written'
