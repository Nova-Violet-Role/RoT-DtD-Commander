...
# companion pass). A count derived from the labels missed M17c and published
# twenty for twenty-one (third pass); M17b fired without a line and was not
# counted at all (fourth pass).
ok() { pass=$((pass+1)); ran=$((ran+1)); echo "PASS $*"; }
...
out=$(run "$T/commands/m1.md"); echo "$out" | grep -q 'ERR  C4' && ok "M1 removed declaration -> C4" || { ko "M1"; echo "$out" | tail -3; fail=$((fail+1)); }
...
out=$(run "$T/commands/m2.md"); echo "$out" | grep -q 'ERR  C8' && ok "M2 (CDATA) model -> C8" || { ko "M2"; fail=$((fail+1)); }
...
out=$(run "$T/commands/m3.md"); echo "$out" | grep -q 'ERR  C5 element orphan' && ok "M3 orphan element -> C5" || { ko "M3"; fail=$((fail+1)); }
...
out=$(run "$T/commands/m4.md"); echo "$out" | grep -q -E 'ERR +C13 .*blank lines' && ok "M4 crammed heading -> C13" || { ko "M4"; echo "$out" | tail -3; fail=$((fail+1)); }
...
out=$(run "$T/commands/m5.md"); echo "$out" | grep -q -E 'ERR +C13 .*no sigil' && ok "M5 heading without sigil -> C13" || { ko "M5"; echo "$out" | tail -3; fail=$((fail+1)); }
...
out=$(run "$T/commands/m6.md"); echo "$out" | grep -q -E 'ERR +C14' && ok "M6 bare colon in front matter -> C14" || { ko "M6"; echo "$out" | tail -3; fail=$((fail+1)); }
...
out=$(run "$T/commands/m7.md"); echo "$out" | grep -q 'ERR  C4' && ok "M7 declaration under IGNORE -> C4" || { ko "M7"; echo "$out" | tail -3; fail=$((fail+1)); }
...
out=$(run "$T/commands/m8.md"); echo "$out" | grep -q 'failed 0' && ok "M8 declaration under INCLUDE keyed by %keep; -> pass" || { ko "M8"; echo "$out" | tail -3; fail=$((fail+1)); }
# Untouched -> pass
out=$(run "$T/commands/pareto-dtd.md"); echo "$out" | grep -q 'failed 0' && ok "M0 untouched file passes" || { ko "M0"; echo "$out" | tail -3; fail=$((fail+1)); }
...
score "$T/m9.md"; rc=$?; [ $rc -eq 1 ] && ok "M9 a fail with a high finding in the element spelling scores as a fail (exit 1)" || { ko "M9 exit=$rc"; fail=$((fail+1)); }
printf '%s\n\n### 🩺 Findings\n\n**high · measured · x:1** planted in the bold spelling\n\nCOMPANION VERDICT: fail\n' "$scope" > "$T/m10.md"
out=$(bash checker/companion-audit.sh --score "$T/m10.md" p a..b opus 2>&1); rc=$?; [ $rc -eq 1 ] && echo "$out" | grep -q 'high findings=0' && echo "$out" | grep -q 'a fail with no high finding' && ok "M10 a bold line is not a finding: high findings=0 and the fail is refused" || { ko "M10 exit=$rc"; fail=$((fail+1)); }
printf '%s\n\n### 🩺 Findings\n\n<finding file="x" line="1" severity="low" confidence="measured">planted</finding>\n\nCOMPANION VERDICT: fail\n' "$scope" > "$T/m11.md"
out=$(bash checker/companion-audit.sh --score "$T/m11.md" p a..b opus 2>&1); rc=$?; [ $rc -eq 1 ] && echo "$out" | grep -q 'a fail with no high finding' && ok "M11 a fail with no high finding breaks LAW.COMPANION.4 (exit 1, named)" || { ko "M11 exit=$rc"; fail=$((fail+1)); }
printf '%s\n\n### 🩺 Findings\n\nnone\n\nCOMPANION VERDICT: pass\n' "$scope" > "$T/m12.md"
score "$T/m12.md"; rc=$?; [ $rc -eq 0 ] && ok "M12 a pass with the scope line scores as a pass (exit 0)" || { ko "M12 exit=$rc"; fail=$((fail+1)); }
...
out=$(bash checker/companion-audit.sh --score "$T/m13.md" p a..b opus 2>&1); rc=$?; [ $rc -eq 1 ] && echo "$out" | grep -q 'LAW.COMPANION.6' && ok "M13 the replaced expression accepted axxb for a..b (landed proof), the whole-line fixed-string match refuses it" || { ko "M13 exit=$rc"; fail=$((fail+1)); }
printf '%s\n\n### 🩺 Findings\n\n<finding file="x" line="1" severity="low" confidence="measured">planted</finding>\n\n### 🩺 Next\n\nRaise the severity="high" on the next run.\n\nCOMPANION VERDICT: fail\n' "$scope" > "$T/m14.md"
out=$(bash checker/companion-audit.sh --score "$T/m14.md" p a..b opus 2>&1); rc=$?; [ $rc -eq 1 ] && echo "$out" | grep -q 'high findings=0' && echo "$out" | grep -q 'a fail with no high finding' && ok "M14 the attribute in prose counts for nothing: high findings=0 and the fail is refused" || { ko "M14 exit=$rc"; fail=$((fail+1)); }
...
out=$(bash checker/companion-audit.sh --score "$T/m15.md" p a..b opus 2>&1); rc=$?; [ $rc -eq 1 ] && echo "$out" | grep -q 'high findings=0' && echo "$out" | grep -q 'a fail with no high finding' && ok "M15 the attribute quoted in a finding body counts for nothing: high findings=0 and the fail is refused" || { ko "M15 exit=$rc"; echo "$out" | tail -2; fail=$((fail+1)); }
printf '%s\n\n### 🩺 Findings\n\n<finding file="x" line="1" severity="high">planted without a confidence</finding>\n\nCOMPANION VERDICT: fail\n' "$scope" > "$T/m16.md"
out=$(bash checker/companion-audit.sh --score "$T/m16.md" p a..b opus 2>&1); rc=$?; [ $rc -eq 1 ] && echo "$out" | grep -q 'LAW.COMPANION.3' && ok "M16 a finding element without its confidence is refused under LAW.COMPANION.3" || { ko "M16 exit=$rc"; echo "$out" | tail -2; fail=$((fail+1)); }
printf '%s\n\n### 🩺 Findings\n\nnone\n\nCOMPANION VERDICT: pass\nCOMPANION VERDICT: pass\n' "$scope" > "$T/m18.md"
out=$(bash checker/companion-audit.sh --score "$T/m18.md" p a..b opus 2>&1); rc=$?; [ $rc -eq 1 ] && echo "$out" | grep -q 'LAW.COMPANION.4 broken, 2 verdict lines' && ok "M18 two verdict lines are refused under LAW.COMPANION.4 (exit 1, named)" || { ko "M18 exit=$rc"; echo "$out" | tail -2; fail=$((fail+1)); }
printf '%s\n\n### 🩺 Findings\n\nnone\n\n### 🩺 Verdict\n\npass, but the line is missing\n' "$scope" > "$T/m19.md"
out=$(bash checker/companion-audit.sh --score "$T/m19.md" p a..b opus 2>&1); rc=$?; [ $rc -eq 1 ] && echo "$out" | grep -q '0 verdict lines' && ok "M19 an answer with no verdict line is refused (exit 1, 0 verdict lines)" || { ko "M19 exit=$rc"; echo "$out" | tail -2; fail=$((fail+1)); }
...
[ $r3 -eq 1 ] && ok "M17c a bare timeout in the allow-list is refused: the ceiling must be the portable one" || { ko "M17c a bare timeout in the allow-list was admitted"; fail=$((fail+1)); }
...
[ $r1 -eq 1 ] && [ $r0 -eq 0 ] && ok "M17 the runner's allow-list carries no writing or spawning tool and every Bash form starts with the portable ceiling; a copy granting Write is refused" || { ko "M17 planted=$r1 real=$r0"; fail=$((fail+1)); }
