...
out=$(mktemp)
ceil 30 node bin/rot-dtd-commander.mjs watch --secs 1 --poll 100 < /dev/null > "$out" 2>&1
rc=$?
...
out=$(mktemp)
ROT_DTD_CEILING=1 ceil 60 node bin/rot-dtd-commander.mjs controls < /dev/null > "$out" 2>&1
rc=$?
...
out=$(mktemp)
ROT_DTD_CEILING=junk ceil 3 node bin/rot-dtd-commander.mjs watch --poll 100 < /dev/null > "$out" 2>&1
rc=$?
