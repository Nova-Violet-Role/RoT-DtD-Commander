...
[![Controls](https://img.shields.io/badge/guards_tripped_on_purpose-30_%2B_21-27ae60?style=flat-square)](#-verify-it-yourself)
...
134 commands, in the families the command index lists, every
...
<!-- rdc-index:begin -->
*134 commands in 15 families, 22 skills, 5 agents. Every family opens below; the rest of this page is folded.*


_This index is generated by `node checker/readme-index.mjs` from the resolved tree; `--check` in the gate refuses a README that disagrees with it. 134 commands in 15 families, 22 skills, 5 agents._
...
[![The Graphic and Geometric Suite](https://img.shields.io/badge/%F0%9F%93%8F_The_Graphic_and_Geometric_Suite-3-0e7490?style=flat-square)](#family-geometry)
...
<details>
<summary><b>📏 The Graphic and Geometric Suite</b> · 3 commands</summary>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/family-the-graphic-and-geometric-suite-dark.svg" />
  <img alt="The Graphic and Geometric Suite: 3 commands with the call each one takes" src="docs/family-the-graphic-and-geometric-suite.svg" />
</picture>
...
For search and filters across all 161 entries at once, open
...
**The Graphic and Geometric Suite** — three commands on one fifty-two rung
ladder in three bands. The surveyor measures a codebase and draws it, and moves
...
<!-- rdc-claims:machine-readable — the same rows the plate above draws, kept as text
     ...
     | every command of the gate script is a run line of the gate workflow or a shell segment of one; a step commented out counts for nothing; the workflow may run more, and that direction is not claimed | `node checker/gate-sync.mjs`: `63 commands in the gate chain, 0 missing from gate.yml`, three controls passing (a run line removed, a step commented out, a file of comments) | every push |
     ...
     | every count the repository publishes (the badges, the tagline, the claims rows, the three manifests) equals the tree: commands, skills, agents, their sum, the Adiutor guards, the checker controls, the declarations | `node checker/counts-sweep.mjs`: `36 places in step`; `--controls` plants a stale badge, a stale count in words and a removed count | every push |
