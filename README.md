...
the gate refuses a README whose index disagrees with the files. It is folded away
so the page stays readable &mdash; open it when you want the catalogue.

<!-- rdc-index:begin -->
<details>
<summary><b>Open the full index</b> &mdash; 131 commands in 14 families, 22 skills, 5 agents</summary>
...
| every command of the gate script is a run line of the gate workflow or a shell segment of one; a step commented out counts for nothing; the workflow may run more, and that direction is not claimed | `node checker/gate-sync.mjs`: `49 commands in the gate chain, 0 missing from gate.yml`, three controls passing (a run line removed, a step commented out, a file of comments) | every push |
