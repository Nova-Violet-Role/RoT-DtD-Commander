...
[![Checker](https://img.shields.io/badge/checked-170_files%2C_0_failed-27ae60?style=flat-square)](#-what-is-claimed-and-the-instrument-behind-each-claim)
[![Contract](https://img.shields.io/badge/contract_audit-2025_declarations%2C_0_unused-27ae60?style=flat-square)](#-what-is-claimed-and-the-instrument-behind-each-claim)
...
<!-- rdc-index:begin -->
*143 commands in 17 families, 22 skills, 5 agents. Every family opens below; the rest of this page is folded.*


_This index is generated by `node checker/readme-index.mjs` from the resolved tree; `--check` in the gate refuses a README that disagrees with it. 143 commands in 17 families, 22 skills, 5 agents._
...
For search and filters across all 170 entries at once, open
...
<!-- rdc-claims:machine-readable — the same rows the plate above draws, kept as text
     ...
     |---|---|---|
     | 143 commands, 22 skills, 5 agents carry a DOCTYPE | `rdc list` | 2026-09-07 |
     | every source passes rules C1 to C16 | `rdc check`: `checked 170  failed 0` | 2026-09-07 |
     | the committed resolved tree equals a fresh build | `rdc build --check`: `307 targets, 0 drifted, 0 failing` | 2026-09-07 |
     ...
     | every declaration in the subsets and the Adiutor contract is used by a source, every law prefix is numbered densely, and every law family is read in ascending order | `node checker/contract-audit.mjs`: `2025 declarations, 0 unused, 0 law gaps`, and the three planted controls | 2026-09-07 |
     ...
     | every subset the repository declares is installed, and a version the recognizer disputes is refused | `rdc doctor` row `subsets`: `33 subsets, every one installed` (run against your own installation, not in CI); `node checker/release-notes.mjs --versions`: the recognised class and version printed beside the manifests | every push |
