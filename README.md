...
[![Checker](https://img.shields.io/badge/checked-173_files%2C_0_failed-27ae60?style=flat-square)](#-what-is-claimed-and-the-instrument-behind-each-claim)
[![Contract](https://img.shields.io/badge/contract_audit-2110_declarations%2C_0_unused-27ae60?style=flat-square)](#-what-is-claimed-and-the-instrument-behind-each-claim)
...
145 commands, in the families the command index lists, every
...
<!-- rdc-index:begin -->
*145 commands in 17 families, 23 skills, 5 agents. Every family opens below; the rest of this page is folded.*


_This index is generated by `node checker/readme-index.mjs` from the resolved tree; `--check` in the gate refuses a README that disagrees with it. 145 commands in 17 families, 23 skills, 5 agents._
...
[![Research](https://img.shields.io/badge/%F0%9F%A4%BF_Research-11-2980b9?style=flat-square)](#family-research)
[![Asking and deciding](https://img.shields.io/badge/%E2%9D%93_Asking_and_deciding-10-8e44ad?style=flat-square)](#family-asking)
...
[![Repository](https://img.shields.io/badge/%F0%9F%90%99_Repository-5-9b59b6?style=flat-square)](#family-repository)
...
<details>
<summary><b>🤿 Research</b> · 11 commands</summary>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/family-research-dark.svg" />
  <img alt="Research: 11 commands with the call each one takes" src="docs/family-research.svg" />
</picture>
...
<details>
<summary><b>❓ Asking and deciding</b> · 10 commands</summary>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/family-asking-and-deciding-dark.svg" />
  <img alt="Asking and deciding: 10 commands with the call each one takes" src="docs/family-asking-and-deciding.svg" />
</picture>
...
<details>
<summary><b>🐙 Repository</b> · 5 commands</summary>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/family-repository-dark.svg" />
  <img alt="Repository: 5 commands with the call each one takes" src="docs/family-repository.svg" />
</picture>
...
<details>
<summary><b>🎓 Skills</b> · 23, each loading itself when its description matches</summary>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/family-skills-dark.svg" />
  <img alt="Skills: 23, each loading itself when its description matches" src="docs/family-skills.svg" />
</picture>
...
For search and filters across all 173 entries at once, open
...
_Generated by `node checker/readme-index.mjs` from the notes on the family rows; `--check` refuses a README that disagrees. 17 families and 2 further groups, 19 in all._
...
**❓ Asking and deciding** · 10 commands
...
**👻 The Phantom Books shelf** · 20 commands
...
**🌌 The RoT MoE lenses** · 10 commands
...
**🪃 File types and dorks** · 11 commands
...
**📏 The Graphic and Geometric Suite** · 5 commands

Measure a codebase, draw it, change it, and produce the graphic the three agreed on, on one ladder of one hundred and eight rungs: three profession bands and four domain bands above them, read through the lens of each profession; the surveyor moves nothing, the architect declares bounds and rewrites nothing, the renovator changes only against a survey and a plan on disk, the generator launches the three or produces, and typography is the contract glyph and plate share.

**⛓️ Inter-operation** · 1 command

One command that reads what you asked for and fires the others it names, in the order it declares. Reach for it when the work crosses families and you would otherwise drive each one by hand.

**💲 The sigil** · 2 commands

What the dollar tokens mean and how one command hands arguments to the next. Reach for these when you do not know what a token will expand to, or which of the collisions across shell, Make and this Suite you are looking at.

**⛔ The lists** · 8 commands
...
**🩺 Workflow and the Adiutor** · 8 commands

The doctor: run it, arm it, read its ledger, compose the workflows it judges. Since 5.0.0 the Adiutor is not armed by default.

**🎓 Skills** · 23

Each loads itself when its description matches what you are doing; none is invoked by name. A skill states what it writes and gates before it writes.

**🕵️ Agents** · 5

Subagents the Suite dispatches with their own context and their own contract. They report findings; the session decides what to do with them.
...
<!-- rdc-claims:machine-readable — the same rows the plate above draws, kept as text
     ...
     |---|---|---|
     | 145 commands, 23 skills, 5 agents carry a DOCTYPE | `rdc list` | 2026-09-07 |
     | every source passes rules C1 to C16 | `rdc check`: `checked 173  failed 0` | 2026-09-07 |
     | the committed resolved tree equals a fresh build | `rdc build --check`: `312 targets, 0 drifted, 0 failing` | 2026-09-07 |
     ...
     | every declaration in the subsets and the Adiutor contract is used by a source, every law prefix is numbered densely, and every law family is read in ascending order | `node checker/contract-audit.mjs`: `2110 declarations, 0 unused, 0 law gaps`, and the three planted controls | 2026-09-07 |
     ...
     | every command of the gate script is a run line of the gate workflow or a shell segment of one; a step commented out counts for nothing; the workflow may run more, and that direction is not claimed | `node checker/gate-sync.mjs`: `86 commands in the gate chain, 0 missing from gate.yml`, three controls passing (a run line removed, a step commented out, a file of comments) | every push |
     ...
     | every count the repository publishes (the badges, the tagline, the claims rows, the three manifests) equals the tree: commands, skills, agents, their sum, the Adiutor guards, the checker controls, the declarations | `node checker/counts-sweep.mjs`: `64 places in step`; `--controls` plants a stale badge, a stale count in words and a removed count | every push |
     ...
     | every subset the repository declares is installed, and a version the recognizer disputes is refused | `rdc doctor` row `subsets`: `38 subsets, every one installed` (run against your own installation, not in CI); `node checker/release-notes.mjs --versions`: the recognised class and version printed beside the manifests | every push |
...
gate opens by proving the workflow runs the same chain (checker/gate-sync.mjs), then runs build --check, check, the thirty-one Adiutor controls, the contract audit, the checker controls, the sweeps, the thirty-five control suites of the gate chain, the slop sweep at zero, the slop measures on README and CHANGELOG (the release ships a changelog section), the tracked-targets sweep, the plates and the release-notes controls; each ends with a line of counts, and the exit code is read directly. Then break it:
...
<!-- rdc-verify:machine-readable — the same commands the plate above draws, kept as
     ...
     bash checker/checker-controls.sh      # M0 to M37: seven mutations refused, one under INCLUDE and the untouched file pass, then thirty-one scorer and runner controls
