...
[![Controls](https://img.shields.io/badge/guards_tripped_on_purpose-30_%2B_22-27ae60?style=flat-square)](#-verify-it-yourself)
...
<!-- rdc-claims:machine-readable — the same rows the plate above draws, kept as text
     ...
     | every command of the gate script is a run line of the gate workflow or a shell segment of one; a step commented out counts for nothing; the workflow may run more, and that direction is not claimed | `node checker/gate-sync.mjs`: `62 commands in the gate chain, 0 missing from gate.yml`, three controls passing (a run line removed, a step commented out, a file of comments) | every push |
