...
[![Controls](https://img.shields.io/badge/guards_tripped_on_purpose-31_%2B_24-27ae60?style=flat-square)](#-verify-it-yourself)
...
<!-- rdc-claims:machine-readable — the same rows the plate above draws, kept as text
     ...
     | every count the repository publishes (the badges, the tagline, the claims rows, the three manifests) equals the tree: commands, skills, agents, their sum, the Adiutor guards, the checker controls, the declarations | `node checker/counts-sweep.mjs`: `61 places in step`; `--controls` plants a stale badge, a stale count in words and a removed count | every push |
...
<!-- rdc-verify:machine-readable — the same commands the plate above draws, kept as
     ...
     bash checker/checker-controls.sh      # M0 to M21: seven mutations refused, one under INCLUDE and the untouched file pass, then fifteen scorer and runner controls
