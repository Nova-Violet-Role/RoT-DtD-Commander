...
[![Contract](https://img.shields.io/badge/contract_audit-2144_declarations%2C_0_unused-27ae60?style=flat-square)](#-what-is-claimed-and-the-instrument-behind-each-claim)
...
<!-- rdc-claims:machine-readable — the same rows the plate above draws, kept as text
     ...
     | every declaration in the subsets and the Adiutor contract is used by a source, every law prefix is numbered densely, and every law family is read in ascending order | `node checker/contract-audit.mjs`: `2144 declarations, 0 unused, 0 law gaps`, and the three planted controls | 2026-09-07 |
     ...
     | every subset the repository declares is installed, and a version the recognizer disputes is refused | `rdc doctor` row `subsets`: `40 subsets, every one installed` (run against your own installation, not in CI); `node checker/release-notes.mjs --versions`: the recognised class and version printed beside the manifests | every push |
