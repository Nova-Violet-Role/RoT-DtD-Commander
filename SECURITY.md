...
| the Scratchpad Companion, a nested `claude -p` the maintainer runs by hand, reads and runs only: its tool allow-list is Read, Grep, Glob and one Bash form, the wrapper `checker/companion-run.sh`, which runs an engine of its table by a reading spelling or a git verb that reads, under the portable ceiling `node lib/ceiling.mjs 300` with stdin closed, and refuses everything else by name; the session runs under `--permission-mode default` with a PreToolUse hook, `checker/companion-guard.mjs`, that refuses any Bash string that is not one wrapper call with no chain or redirect, any Bash call whose own run_in_background field is set, and every writing tool by name (the grant itself is a prefix on a shell string); HEAD, the porcelain status and every ignored file of the tree, minus only the runner's own stream, are compared before and after the session | controls M17, M17b, M17c and M23 to M36 in `checker/checker-controls.sh`: a copy of the runner granting Write, a bare Bash, a bare timeout, the old node and git prefixes or no permission mode is refused; the wrapper's refusals are tripped; every granted spelling is walked under the tree state; a changed tree, at the root, under an ignored artifact directory, under dist/ or under the index cache, a fired ceiling and a suite inside a suite are read as such; the scorer's fail path is proved by a gutted copy |
