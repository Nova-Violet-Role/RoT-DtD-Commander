...
<!-- end subset cc-ask -->
  ...
  <!ATTLIST meta_forge schematic (callout) #FIXED "callout">
  ...
  <!ENTITY LAW.META.1 "The schematic of this command is callout, fixed in its DOCTYPE; the file it writes is the GitHub callout shape and every syntax in it comes from a SCHEMA.callout.* entity (LAW.SCHEMA.1).">
  <!ENTITY LAW.META.2 "The sections are those of SCHEMA.meta.sections, rendered in that order; the argument words are embedded through SCHEMA.callout.reference and SCHEMA.callout.literal in the class the intake chose (LAW.SCHEMA.2, LAW.SCHEMA.3).">
  ...
  <!ENTITY LAW.META.4 "The file takes the extension SCHEMA.ext.callout, carries the chosen SPDX identifier where its form allows a comment, and passes every cc-form guard of its kind before it is reported (LAW.SCHEMA.4).">
...
<objective>
Create a meta-prompt, a prompt that writes prompts for <quoted trust="cdata" source="user-args">$ARGUMENTS</quoted> (or ask what it is for), written in the GitHub callout shape.

The schematic is pinned: a quoted line under a typed callout, one of the five types per case. What a literal is, what expands, how a value is referenced, defined, escaped, commented, included or made conditional, is read from the SCHEMA.callout.* table of cc-schematic.dtd, the table cut from the argument-variant references: the quoted heredoc is the CDATA section is the strip block scalar is the NestedText multiline string, and the argument string is always the quoted whole. The sections are SCHEMA.meta.sections. The file is guarded by cc-form before it is reported and proven by a planted out-of-table syntax that the proof refuses.
</objective>
...
4. Render the `sections`: one `section` per name of SCHEMA.meta.sections, in order, each with its text; render the `schemas`: one `semantic` per schema chosen, its `part` elements from the SEMANTIC entity of that schema with occurs one, optional or many, each rendered by SEMANTIC.callout.part, SEMANTIC.callout.many and SEMANTIC.callout.label (LAW.META.6); render the `embedding`: the reference syntax SCHEMA.callout.reference, the literal syntax SCHEMA.callout.literal, and the cc-args class chosen for the argument words.
5. Write the `file` <name>.<schematic>.md: the GitHub callout shape, the sections in order, every concept in the syntax the table declares, the SPDX header where a comment is allowed, UTF-8 LF without BOM; re-read it and render path and bytes (LAW.META.4).
...
- `schemas`: **🗯️ Schemas**, one line per semantic schema chosen with its parts in order, or one line saying none
...
- `assumption_made`: **🗯️ Assumptions Made**, every ASK.META.* question not asked, with the first option taken
</grammar_map>
...
- [refentry|qandaset|procedure|glossary|textdesc|msgset|productionset]: parts [in order, occurs one, optional or many], rendered by the callout part, many and label rules; or: none, the sections alone
