---
description: "DTD-native: audit a slash command file here, in the foreground: the contract rules C1 to C16 through the checker under a ceiling, then the style areas of slash-command-auditor-dtd read from its agent file as data and checked one by one; findings with file and line, one verdict; no subagent is summoned"
