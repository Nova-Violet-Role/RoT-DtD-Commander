...
4. Render the `sections`: one `section` per name of SCHEMA.meta.sections, in order, each with its text; render the `schemas`: one `semantic` per schema chosen, its `part` elements from the SEMANTIC entity of that schema with occurs one, optional or many, each rendered from the schema's cell for polyglot: print its skeleton with node lib/schematic.mjs render, the schema name and polyglot, and fill the bracketed words in place (LAW.META.6); render the `forms`: one `form` per kind chosen by ASK.FORM.1 and ASK.FORM.2 with its variant and expansion no, nt alone when none was chosen (LAW.META.7); render the `embedding`: the reference syntax SCHEMA.polyglot.reference, the literal syntax SCHEMA.polyglot.literal, and the cc-args class chosen for the argument words.
...
- [refentry|qandaset|procedure|glossary|textdesc|msgset|productionset]: parts [in order, occurs one, optional or many], rendered from its polyglot cell; or: none, the sections alone
