...
<output_format>
<grammar_map>
Render the `ten_ten_ten` root declared in the DOCTYPE as the markdown below. One declared element per heading, in declared order; a required element with nothing to say still appears, with one line saying so.
- `decision`: **Decision**
