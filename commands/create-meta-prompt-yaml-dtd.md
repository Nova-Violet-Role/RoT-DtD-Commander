...
4. Render the `sections`: one `section` per name of SCHEMA.meta.sections, in order, each with its text; render the `embedding`: the reference syntax SCHEMA.yaml.reference, the literal syntax SCHEMA.yaml.literal, and the cc-args class chosen for the argument words.
