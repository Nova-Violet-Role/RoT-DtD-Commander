...
4. Render the `sections`: one `section` per name of SCHEMA.prompt.sections, in order, each with its text; render the `schemas`: one `semantic` per schema chosen, its `part` elements from the SEMANTIC entity of that schema with occurs one, optional or many, each rendered by SEMANTIC.callout.part, SEMANTIC.callout.many and SEMANTIC.callout.label (LAW.PROMPT.6); render the `embedding`: the reference syntax SCHEMA.callout.reference, the literal syntax SCHEMA.callout.literal, and the cc-args class chosen for the argument words.
