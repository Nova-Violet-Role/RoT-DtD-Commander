...
<!-- ===== LAWS ===== -->
<!ENTITY LAW.STAR.1 "Every manager is a declaration of STAR.managers, not a branch in code: its binary, its search subcommand, its install subcommand and its ceiling come from its entity, and a seventh manager is one more entity and no new code.">
...
The declarations this command reads: STAR.managers and the six adapters STAR.mgr.scoop, STAR.mgr.chocolatey, STAR.mgr.bun, STAR.mgr.vcpkg, STAR.mgr.cargo and STAR.mgr.uv; STAR.absent for what a silent binary means; STAR.ceiling.search for the bound on every probe; STAR.file for where the list lands; and LAW.STAR.1 for why a seventh manager is a declaration and no new code.
</objective>
