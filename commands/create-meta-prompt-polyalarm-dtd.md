---
description: "DTD-native: create a meta-prompt, a prompt that writes prompts written in a polyglot whose Markdown layer is the alarm shape through twelve questions in three rounds; every syntax comes from the SCHEMA.polyalarm.* table, the argument words are embedded in a declared class, the file passes the form guards, and a proof plants one out-of-table syntax and shows it refused"
...
<!-- end subset cc-ask -->
  ...
  <!ATTLIST meta_forge schematic (polyalarm) #FIXED "polyalarm">
  ...
  <!ENTITY LAW.META.1 "The schematic of this command is polyalarm, fixed in its DOCTYPE; the file it writes is a polyglot whose Markdown layer is the alarm shape and every syntax in it comes from a SCHEMA.polyalarm.* entity (LAW.SCHEMA.1).">
  <!ENTITY LAW.META.2 "The sections are those of SCHEMA.meta.sections, rendered in that order; the argument words are embedded through SCHEMA.polyalarm.reference and SCHEMA.polyalarm.literal in the class the intake chose (LAW.SCHEMA.2, LAW.SCHEMA.3).">
  ...
  <!ENTITY LAW.META.4 "The file takes the extension SCHEMA.ext.polyalarm, carries the chosen SPDX identifier where its form allows a comment, and passes every cc-form guard of its kind before it is reported (LAW.SCHEMA.4).">
  ...
  <!ENTITY LAW.META.6 "Every semantic schema chosen by ASK.SCHEMA.1 and ASK.SCHEMA.2 is rendered as a semantic element whose parts are those of its SEMANTIC entity in order, by its cell for polyalarm, the SEMANTIC entity named by the schema and then polyalarm, whose skeleton node lib/schematic.mjs render prints; a required part missing is a failed answer (LAW.SCHEMA.6, LAW.SCHEMA.7, LAW.SCHEMA.8, LAW.SCHEMA.9).">
...
<objective>
Create a meta-prompt, a prompt that writes prompts for <quoted trust="cdata" source="user-args">$ARGUMENTS</quoted> (or ask what it is for), written in a polyglot whose Markdown layer is the alarm shape.

The schematic is pinned: YAML front matter naming the parts, then house callouts, each layer literal to the one outside it. What a literal is, what expands, how a value is referenced, defined, escaped, commented, included or made conditional, is read from the SCHEMA.polyalarm.* table of cc-schematic.dtd, the table cut from the argument-variant references: the quoted heredoc is the CDATA section is the strip block scalar is the NestedText multiline string, and the argument string is always the quoted whole. The sections are SCHEMA.meta.sections. The file is guarded by cc-form before it is reported and proven by a planted out-of-table syntax that the proof refuses.
</objective>
...
- [schema, one of the SEMANTIC families docbook, dita, tei or data]: parts [in order, occurs one, optional or many], rendered from its polyalarm cell; or: none, the sections alone
...
reference: [SCHEMA.polyalarm.reference]; literal: [SCHEMA.polyalarm.literal]; class [pcdata|cdata|ndata|section]

### 🪅 File

`<name>.polyalarm.md` ([bytes] B, LF, no BOM)
