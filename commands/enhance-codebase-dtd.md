...
<!-- begin subset enhance-codebase -->
<!-- SPDX-License-Identifier: AGPL-3.0-or-later OR EUPL-1.2 -->
<!-- Copyright 2026 Saimonokuma. -->
<!--
  enhance-codebase.dtd : the variant subset of /enhance-codebase-dtd.
...
<!-- ===== THE BAND, PINNED ===== -->
<!ATTLIST enhance_run
          band     CDATA #FIXED "6-10"
          verbs    CDATA #FIXED "heighten, promote, cultivate, enhancement and upgrade"
          hands_to CDATA #FIXED "overhaul-codebase-dtd">

<!-- Every verb of this band, so a possibility's verb attribute can be read
     against the declaration rather than against memory. -->
<!ENTITY ENHANCE.band "6|7|8|9|10">
<!ENTITY ENHANCE.low "6">
<!ENTITY ENHANCE.high "10">
<!ENTITY ENHANCE.next "11">
<!ENTITY ENHANCE.what "a capability the codebase implied becomes actually there, and a habit becomes a declaration">
...
<!-- The argument reaches this command as an unparsed channel and is walked by
     ...
     directory name. -->
<!ENTITY ENHANCE.guard.quoting "the argument is split like shell words and never evaluated; a path with a space stays one word and a word that looks like a flag inside a quoted path stays part of the path">
...
<!ENTITY ENHANCE.guard.literal "text written into the study is literal: a dollar sign, a backtick, a brace or a heredoc marker inside a finding is written as it was read and never expanded">
...
<!ENTITY ENHANCE.guard.pcdata "a possibility's text is escaped into PCDATA, never wrapped in a CDATA section, because a fragment carrying the CDATA close delimiter would end the section early">
...
<!ENTITY ENHANCE.guard.pentity "a parameter entity found in a scanned file is reported as data and never expanded, so a foreign tree cannot inject declarations into this run">

<!-- ===== THE LAWS OF THIS VARIANT ===== -->
<!ENTITY LAW.ENHANCE.1 "This command exposes only the verbs of ENHANCE.band, which are heighten, promote, cultivate, enhancement and upgrade; the band is a #FIXED attribute on enhance_run, so an answer that claims another band is invalid against this subset, and a possibility above the band is rendered as next_verb naming ENHANCE.next and the command overhaul-codebase-dtd (LAW.AMP.4).">
<!ENTITY LAW.ENHANCE.2 "The four guards ENHANCE.guard.quoting, ENHANCE.guard.literal, ENHANCE.guard.pcdata and ENHANCE.guard.pentity hold for every run: the argument is walked and never evaluated, the study is written literally, every possibility is escaped into PCDATA, and a parameter entity found in a scanned file is data.">
<!ENTITY LAW.ENHANCE.3 "The intake runs at most five rounds of four questions, and its page grows: the size is what the operator earned, held under a ceiling the size of the walk allows, and it is carried in the state record so a later run starts where the answering left it (LAW.AMP.5, LAW.AMP.11).">
<!-- end subset enhance-codebase -->

  <!ELEMENT enhance_run (args, intake, walk, generator, study, release, next_verb, assumption_made*)>
  <!ENTITY LAW.ENHANCE.4 "The page the generator offers is the size the answering earned, resumed from the state record and held under the ceiling the walk's own size allows; a run that opens on a fresh target starts at AMP.page and a practised operator is offered more (LAW.AMP.11).">
  <!ENTITY LAW.ENHANCE.5 "The intake runs five rounds of at most four questions, twenty in all, and the rounds grow: round one is drawn from the walk, and every later round is generated from what the previous answers opened, so a round that would ask nothing new ends the intake early rather than asking to fill its count (LAW.ASK.6, LAW.ASK.11).">
  <!ENTITY LAW.ENHANCE.6 "A run is bounded and the sequence of runs is not: the state record makes every later invocation continue where this one stopped, a refusal returns only as a reopen once its runs are up or a file beneath it moved, and a possibility marked done never returns (LAW.AMP.6, LAW.AMP.12).">
]>
...
<objective>
Grow the codebase at <quoted trust="cdata" source="user-args">$ARGUMENTS</quoted> (or the repository of the working directory if no arguments provided) by the enhance band of the ladder: a capability the codebase implied becomes actually there, and a habit becomes a declaration.
...
metamorphosis, with AMP.ladder.count saying how many there are. `enhance-codebase.dtd`
pins this command's share of it: ENHANCE.band lists the verb numbers, ENHANCE.low and
ENHANCE.high its ends, ENHANCE.next the verb above it, ENHANCE.what the kind of change it
...
Four guards hold wherever this command reads or writes: ENHANCE.guard.quoting, so a
target path carrying a space is one word and never two; ENHANCE.guard.literal, so a
dollar sign, a brace, a backtick or a heredoc marker inside a finding survives
into the study byte for byte; ENHANCE.guard.pcdata, so a possibility's text is escaped
into PCDATA and never wrapped in a CDATA section, because a fragment carrying
the close delimiter would end the section early; and ENHANCE.guard.pentity, so a
...
2. Read the state record first: `node lib/ceiling.mjs 60 node lib/amplify.mjs state` in the target. Its run number, its generator offset, the verb the last run ended on and every closed id are the memory this run continues from; a target with no record starts at run 0 (LAW.AMP.6).
3. Detect the layers: `node lib/ceiling.mjs 60 node lib/amplify.mjs detect`. A target that declares none of them walks the generic layer alone and the `walk` element carries declared no (LAW.AMP.10).
4. Walk them: `node lib/ceiling.mjs 900 node lib/amplify.mjs walk` in the foreground, reading each exit code directly. Render one `layer` element per layer with its instrument, its exit, its read of its of, and walked yes, no or timeout; instruments before any reading by hand, and a layer that reached its ceiling says timeout, never no (LAW.AMP.1, LAW.AMP.2, LAW.AMP.9).
