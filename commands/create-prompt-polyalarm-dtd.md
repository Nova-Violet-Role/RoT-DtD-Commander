...
<!-- end subset cc-ask -->
  ...
  <!ATTLIST prompt_forge schematic (polyalarm) #FIXED "polyalarm">
  ...
  <!ENTITY LAW.PROMPT.1 "The schematic of this command is polyalarm, fixed in its DOCTYPE; the file it writes is a polyglot whose Markdown layer is the alarm shape and every syntax in it comes from a SCHEMA.polyalarm.* entity (LAW.SCHEMA.1).">
  <!ENTITY LAW.PROMPT.2 "The sections are those of SCHEMA.prompt.sections, rendered in that order; the argument words are embedded through SCHEMA.polyalarm.reference and SCHEMA.polyalarm.literal in the class the intake chose (LAW.SCHEMA.2, LAW.SCHEMA.3).">
  ...
  <!ENTITY LAW.PROMPT.4 "The file takes the extension SCHEMA.ext.polyalarm, carries the chosen SPDX identifier where its form allows a comment, and passes every cc-form guard of its kind before it is reported (LAW.SCHEMA.4).">
...
<objective>
Create a prompt for <quoted trust="cdata" source="user-args">$ARGUMENTS</quoted> (or ask what it is for), written in a polyglot whose Markdown layer is the alarm shape.

The schematic is pinned: YAML front matter naming the parts, then house callouts, each layer literal to the one outside it. What a literal is, what expands, how a value is referenced, defined, escaped, commented, included or made conditional, is read from the SCHEMA.polyalarm.* table of cc-schematic.dtd, the table cut from the argument-variant references: the quoted heredoc is the CDATA section is the strip block scalar is the NestedText multiline string, and the argument string is always the quoted whole. The sections are SCHEMA.prompt.sections. The file is guarded by cc-form before it is reported and proven by a planted out-of-table syntax that the proof refuses.
</objective>
...
4. Render the `sections`: one `section` per name of SCHEMA.prompt.sections, in order, each with its text; render the `schemas`: one `semantic` per schema chosen, its `part` elements from the SEMANTIC entity of that schema with occurs one, optional or many, each rendered from the schema's cell for polyalarm: print its skeleton with node lib/schematic.mjs render, the schema name and polyalarm, and fill the bracketed words in place (LAW.PROMPT.6); render the `forms`: one `form` per kind chosen by ASK.FORM.1 and ASK.FORM.2 with its variant and expansion no, nt alone when none was chosen (LAW.PROMPT.7); render the `embedding`: the reference syntax SCHEMA.polyalarm.reference, the literal syntax SCHEMA.polyalarm.literal, and the cc-args class chosen for the argument words.
5. Write the `file` <name>.<schematic>.md: a polyglot whose Markdown layer is the alarm shape, the sections in order, every concept in the syntax the table declares, the SPDX header where a comment is allowed, UTF-8 LF without BOM; re-read it and render path and bytes (LAW.PROMPT.4).
