...
- 145 commands, 23 skills, 5 agents; checked 173; 2144 declarations; 86 gate-chain commands; 312 build targets, 0 drifted
- `node checker/contract-audit.mjs`: 2144 declarations, 0 unused, 0 law gaps
- `node bin/adiutor.mjs controls`: 31 run, 0 failing; `node lib/cache.mjs controls`: 20 passed, 0 failed; `node lib/amplify.mjs controls`: 36 run, 0 failing
...
- `node checker/subsets-sweep.mjs --controls`: 6 run, 0 failing; `node checker/counts-sweep.mjs --controls`: 7 run, 0 failing
- **Where convergence stops.** `dtd/` declares what can be declared; `checker/*.mjs` asserts what cannot: co-occurrence constraints, context-dependent content and datatypes on text stay instruments, never entities. `evidence`, `step` and `term` live once in `cc-core`; `entry`, `section` and `walk` keep their family models in disjoint trees, and the subsets sweep fails any resolved tree that reads one element under two models.
...
- `node checker/contract-audit.mjs`: 2127 declarations, 0 unused, 0 law gaps
...
- `node bin/rot-dtd-commander.mjs check`: checked 173, failed 0
