...
//
// Usage: node lib/cache.mjs save <command> --state <state.nt> [--root <dir>] [--reason user|size|token]
...
// SCHEMA.nt.literal: a multiline string is an angle bracket per line. A
// value longer than one line of prose is folded at a word boundary so the
// file reads as the schematic says, a block of bracketed lines a reader
// takes in at a glance, never one line that scrolls; the fold is part of
// the value, so what is written is what is read back. The first cache of
// 9.0.0 wrote every value on one line and the operator read it as a file
// the schematic had never touched.
export const LINE = 80;
export function fold(s, width = LINE) {
  const text = String(s);
  if (/\n/.test(text) || text.length <= width) return text;
  const lines = [];
  let cur = '';
...
export function stateToFields(command, state, { now = new Date(), reason = 'user' } = {}) {
  ...
  const answers = {};
  s.answers.forEach((a, i) => { answers[`${i + 1} ${a.header}`] = fold(a.answer); });
  const data = {
    command: c,
    saved: new Date(now).toISOString(),
    reason,
    task: fold(s.task),
    slots: Object.fromEntries(Object.entries(s.slots).map(([k, v]) => [k, fold(v)])),
    answers,
    gate,
    next: fold(s.next),
  };
...
export function controls() {
  // every path the controls print is relative to a temp root, never the tree
  // (the twenty-third companion pass read them as tree paths)
  const rows = [];
  let pass = 0;
  let fail = 0;
  ...
  try {
    ...
    control('a value longer than one line is written as the literal the nt schematic declares, an angle bracket per line, and reads back as written', () => {
      const long = 'Close the release: ' + 'fix, build, check, sweep, commit, gate, companion, '.repeat(6) + 'then tag.';
      const r = save('fold-dtd', { ...sampleState(), task: long, next: 'read the record' }, { root });
      const text = readFileSync(cachePath('fold-dtd', root), 'utf8');
      const bracketed = text.split('\n').filter((l) => /^  > /.test(l)).length;
      const back = load('fold-dtd', { root }).data.task;
      const widest = Math.max(...text.split('\n').map((l) => l.length));
      return { ok: r.state === 'saved' && bracketed >= 4 && back === fold(long) && back.replace(/\n/g, ' ') === long && widest <= LINE + 4 && /^task:$/m.test(text), detail: `${bracketed} bracketed lines, widest ${widest}, ${r.bytes} bytes` };
    });
    control('CACHE.dir is ignored by the repository this module lives in, so a save inside the work tree never reaches a commit (LAW.CACHE.6)', () => {
      const repo = join(HERE, '..');
      const probe = `${CONTRACT.dir}/probe-${process.pid}.nt`;
      const r = spawnSync('git', ['check-ignore', '-q', probe], { cwd: repo, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], timeout: 30000 });
      return { ok: r.status === 0, detail: `git check-ignore ${probe}: exit ${r.status === null ? 'none' : r.status}` };
    });
