...
export function stateToFields(command, state, { now = new Date(), reason = 'user' } = {}) {
  ...
  for (const a of s.answers) {
    if (!a || typeof a.header !== 'string' || typeof a.answer !== 'string') throw new Error('an answer without a header and an answer string');
  }
  const g = s.gate || {};
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    if (verb === 'controls') {
      ...
      if (!command || !stateFile) throw new Error('usage: save <command> --state <state.json> [--root <dir>] [--reason user|size|token]');
      ...
      console.log('usage: node lib/cache.mjs save <command> --state <state.json> [--root <dir>] [--reason user|size|token] | load <command> | consume <command> | controls');
      process.exit(2);
    }
  } catch (e) {
