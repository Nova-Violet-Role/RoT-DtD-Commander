...
export function shimScript(shimPath, text = null) {
  ...
  const m = /"%(?:dp0|~dp0)%?\\([^"\r\n]+\.(?:m?js|cjs))"/i.exec(t);
  return m ? join(dirname(shimPath), m[1]) : null;
}
...
export function runUnder(seconds, cmd, args, { stdio = 'inherit', onSpawn = null } = {}) {
  return new Promise((done) => {
    ...
    let script = null;
    if (win && /\.(cmd|bat)$/i.test(exe)) { try { script = shimScript(exe); } catch { script = null; } }
    const viaShell = win && /\.(cmd|bat)$/i.test(exe) && !script;
    if (viaShell && args.some((a) => /[\r\n"]/.test(String(a)))) return done({ code: 2, fired: false, error: `${exe} is a shell shim that names no node script, and an argument carries a newline or a quote the shell would cut; refused rather than truncated` });
    const child = script
      ? spawn(process.execPath, [script, ...args], { stdio })
      : viaShell
...
export async function controls(io = console) {
  ...
  say(/node_modules[\\/]probe[\\/]cli\.js$/.test(parsed) && whole, `an npm shim resolves to the node script it wraps (${parsed.replace(/^.*probe-shim\.cmd/, '<dir>')}) and a two-line argument survives ${how}`);
