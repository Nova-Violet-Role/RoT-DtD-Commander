...
export function controls(io = console) {
  ...
  if (c.laws.length !== 6) fail.push('LAW.SLOP.* count ' + c.laws.length + ', expected exactly 6; teach the code before adding a law');
...
if (isMain) {
  ...
  if (argv[0] === 'controls') process.exit(controls() ? 0 : 1);
    ...
    console.log(NL + 'slop sweep: ' + files.length + ' files, ' + bad + ' slop' + (max ? ', baseline ' + max + ' (may only shrink)' : ''));
    process.exit(bad > max ? 1 : 0);
