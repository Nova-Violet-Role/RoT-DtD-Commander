...
export function unreached(root = ROOT) {
  ...
  for (const p of files(root)) {
    ...
    // A reason recorded in the file is the same escape LAW.RER.3 allows.
    const reasoned = /\b(kept outside|outside the gate|on purpose|deliberately|because|reason)\b/i.test(text.slice(0, 4000));
...
function main() {
  ...
  console.log('regression: claims | retenue | unreached | diff <from> <to> | controls');
  process.exit(2);
}
