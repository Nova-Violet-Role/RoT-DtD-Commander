...
//   unreached             report every controls verb the gate never runs (LAW.RER.4)
...
// ---------- LAW.RER.4: a mechanic implemented and never exercised ----------
// A module that carries a controls verb the gate never runs is a guard nobody
// has tripped. Resolve `npm run <script>` transitively first: the gate chain
// names scripts, not paths, and a grep for the path finds nothing.
export function expandScripts(scripts, cmd, depth = 0) {
  if (depth > 8) return cmd;
  return cmd.replace(/npm run ([\w:.-]+)/g, (m, name) => (scripts[name] ? expandScripts(scripts, scripts[name], depth + 1) : m));
}

// The marker and the words it may carry, read from the DTD (LAW.RER.6).
export const MARKER = (/<!ENTITY\s+RER\.outside\.marker\s+"([^"]*)"/.exec(DTD) || [, 'rer-outside:'])[1];
export const OUTSIDE = ((/<!ENTITY % rer\.outside\s+"\(([^)]*)\)"/.exec(DTD) || [, ''])[1]).split('|').filter(Boolean);

export function unreached(root = ROOT) {
  const pkgPath = join(root, 'package.json');
  if (!existsSync(pkgPath)) return [];
  const scripts = (JSON.parse(readFileSync(pkgPath, 'utf8')).scripts) || {};
  if (!scripts.gate) return [];
  const full = expandScripts(scripts, scripts.gate);
  const out = [];
  for (const p of files(root)) {
    if (!p.endsWith('.mjs')) continue;
    const rel = relative(root, p).replace(/\\/g, '/');
    const text = readFileSync(p, 'utf8');
    if (!/function controls|['"]controls['"]/.test(text)) continue;
    if (full.includes(rel)) continue;
    // LAW.RER.6: the escape is an enumeration, not prose. The first cut of this
    // arm accepted any header containing "because", which is the bound-in-prose
    // this release wrote LAW.ASK.15 to condemn.
    const marker = new RegExp(MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\s*(\\w+)').exec(text.slice(0, 4000));
    const reasoned = !!(marker && OUTSIDE.includes(marker[1]));
    if (reasoned) continue;
    out.push({
      kind: 'retenue', subject: rel, where: rel + ':1',
      severity: 'medium', confidence: 'measured',
      text: 'carries a controls verb the expanded gate never runs, and records no reason for staying outside it',
    });
  }
  return out;
}
...
// Every shape this instrument declares, read back out of the DTD. A gloss no
// caller prints is a gloss nobody can check, which is LAW.RER.4 applied to
// documentation instead of to code.
export function contract() {
  const L = [];
  const one = (n) => {
    const m = new RegExp('<!ENTITY\\s+' + n.replace(/\./g, '\\.') + '\\s+"([^"]*)"').exec(DTD);
    return m ? m[1] : null;
  };
  L.push('root elements: regression_run, companion_audit is the sibling shape in checker/companion-audit.dtd');
  L.push('verdicts: ' + VERDICT_CLEAN + ' | ' + VERDICT_FINDINGS);
  for (const n of ['RER.claim.reads', 'RER.claim.reader', 'RER.vestigial']) L.push(n + ': ' + (one(n) || '(undeclared)'));
  L.push('outside marker: ' + MARKER + ', one of ' + OUTSIDE.join(', '));
  for (const w of OUTSIDE) L.push('  ' + w + ': ' + (one('RER.outside.' + w) || '(no gloss declared)'));
  return L;
}
...
function controls() {
  ...
  try {
    ...
    // 11 and 12. LAW.RER.4, which governed nothing until this arm existed.
    // The transitive expansion is the control that matters: without it a gate
    // naming `npm run controls:x` looks like it runs nothing at all.
    const sc = { gate: 'node a.mjs && npm run mid', mid: 'npm run deep', deep: 'node lib/reached.mjs controls' };
    say(expandScripts(sc, sc.gate).includes('lib/reached.mjs controls'),
      'trip: npm run is expanded transitively, so a script three levels deep is still reached: ' + expandScripts(sc, sc.gate));
    say(!expandScripts(sc, sc.gate).includes('npm run'),
      'no npm run token survives the expansion, so a path grep can be trusted afterwards');
...
function main() {
  ...
  if (verb === 'unreached') {
    const f = unreached();
    process.exit(render(f, 'scope files=' + files().length + ' arm=unreached'));
  }
  if (verb === 'retenue') {
    const f = retenue();
  ...
  console.log('regression: claims | retenue | unreached | contract | diff <from> <to> | controls');
  process.exit(2);
}
