...
// flag, and one process over 789 files is faster than three greps.
//
...
export function judge(buf) {
  ...
  return { cr, ctl, bom, ok: cr === 0 && ctl === 0 && !bom };
}
...
export function controls(root = ROOT, io = console) {
  ...
  try {
    ...
    say(Object.keys(plants).every((f) => files.includes(f)), 'the three planted untracked files are enumerated');
    const r = sweep(root, Object.keys(plants));
    const kinds = r.bad.map((b) => b[0]).sort().join(',');
    say(kinds === 'BOM,CR,CTRL', `trip: a planted CR, control byte and BOM are each detected: ${kinds || 'none'}`);
  ...
  io.log(`encoding controls: 5 run, ${fail} failing`);
  return fail === 0;
}
