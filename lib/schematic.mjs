...
export function calloutType(p) {
  ...
  if (/constraint|prerequisite/.test(p.name)) return 'WARNING';
...
export function table() {
  const out = ['<!-- SPDX-License-Identifier: AGPL-3.0-or-later OR EUPL-1.2 -->', '<!-- Copyright 2026 Saimonokuma. -->', '',
    ...
    `${SCHEMAS.length} schemas, ${FORMS.length} forms (the six schematics and the cc-form kinds beyond them, md being the callout schematic), ${SCHEMAS.length * FORMS.length} cells. A bracketed word is where the part's text goes; a part that occurs many is shown twice; a part that occurs optional is preceded by the form's comment saying when given, except in JSON, which has no comment.`, ''];
  for (const s of SCHEMAS) {
    const ps = parts(s);
...
if (isMain) {
  ...
  if (a === 'controls') {
    ...
    console.log(`schematic controls: ${r.bad ? `${r.bad} failing` : 'ok (0 failing)'}, schemas ${SCHEMAS.length}, forms ${FORMS.length}, cells ${SCHEMAS.length * FORMS.length}, concepts ${CONCEPTS.length}`);
  ...
  else { console.log('usage: node lib/schematic.mjs render <schema> <form> | parts <schema> | cell <schema> <form> | table | controls'); process.exit(2); }
}
