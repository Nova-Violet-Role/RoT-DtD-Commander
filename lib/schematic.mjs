...
const KIND = { callout: 'md', heredoc: 'heredoc', yaml: 'yaml', nt: 'nt', xml: 'xml', polyglot: 'polyglot', jmd: 'jmd', json: 'json', toml: 'toml', alarm: 'alarm', polyalarm: 'polyalarm' };
const FENCE = { callout: 'md', heredoc: 'sh', yaml: 'yaml', nt: 'nt', xml: 'xml', polyglot: 'md', jmd: 'md', json: 'json', toml: 'toml', alarm: 'md', polyalarm: 'md' };
export const ALARM_TYPES = (entOf(DTD, 'SEMANTIC.alarm.types') || '').match(/\b[A-Z]{3,}\b/g) || [];
...
// SEMANTIC.alarm.types: the house vocabulary of the alarm form, one type per
// kind of part; the cells name the same type per part and matrixCheck holds
// them to it.
export function alarmType(p) {
  if (/question/.test(p.name)) return 'QUESTION';
  if (/answer|result|resolution/.test(p.name)) return 'ANSWER';
  if (/constraint|prereq|rule/.test(p.name)) return 'LAW';
  if (/example/.test(p.name)) return 'OUTPUT';
  if (/step|command|synopsis/.test(p.name)) return 'PROMPT';
  if (/check|verify|test/.test(p.name)) return 'CHECKS';
  if (/^(title|name|refname|glossterm|term)$/.test(p.name)) return 'FRAMEWORK';
  if (p.occurs === 'one') return 'ALARM';
  return 'NOTE';
}
...
export function render(schema, form) {
  ...
  switch (form) {
    ...
    case 'alarm':
      ...
      break;
    case 'polyalarm':
      out.push('---', `schema: ${schema}`, 'form: polyalarm', 'parts:');
      for (const p of ps) out.push(`  - ${p.name}${p.occurs === 'one' ? '' : ' # ' + p.occurs}`);
      out.push('---', '', render(schema, 'alarm').replace(/\n+$/, ''));
      break;
    case 'jmd':
      for (const p of ps) {
        opt(p, `<!-- ${p.name}: when given -->`);
...
export function readBack(text, form) {
  ...
  if (form === 'callout' || form === 'alarm') each(/^>\s*\[!([A-Za-z]+)\]\s+([a-z_]+)/, 2);
  ...
  } else if (form === 'polyglot' || form === 'polyalarm') {
...
export function matrixCheck(dtd = DTD) {
  ...
  for (const s of SCHEMAS) {
    ...
    for (const form of forms) {
      ...
      for (const p of ps) {
        const want = form === 'callout' ? `${calloutType(p)} ${p.name}` : form === 'alarm' ? `${alarmType(p)} ${p.name}` : form === 'heredoc' ? p.name.toUpperCase() : p.name;
...
export function controls() {
  ...
  for (const s of SCHEMAS) {
    ...
    for (const form of FORMS) {
      ...
      if (same && (form === 'polyglot' || form === 'polyalarm')) {
        const body = text.slice(text.indexOf(NL + '---' + NL, 4) + 5);
        same = JSON.stringify(readBack(body, form === 'polyglot' ? 'callout' : 'alarm')) === JSON.stringify(want);
        if (!same) notes.push(form + ' body drifts from its front matter');
      }
      if (same && form === 'json') {
  ...
  const alarmUsed = new Set();
  for (const s of SCHEMAS) for (const p of parts(s)) alarmUsed.add(alarmType(p));
  const strangeAlarm = [...alarmUsed].filter((t) => !ALARM_TYPES.includes(t));
  row(strangeAlarm.length === 0 && alarmUsed.size >= 5, `every alarm type rendered is one of SEMANTIC.alarm.types (${ALARM_TYPES.join(', ')}), ${alarmUsed.size} in use${strangeAlarm.length ? '; strange: ' + strangeAlarm.join(', ') : ''}`);
  const foo = guards(render('refentry', 'alarm').split('[!FRAMEWORK] refname').join('[!FOO] refname'), 'alarm').find((g) => g.name === 'alarm');
  row(foo && !foo.held, `trip: a callout type outside the house vocabulary is refused by the alarm guard: ${foo ? foo.detail : 'guard not applied'}`);
