...
// Node's DEP0190 warns that args with shell:true are concatenated, not
// escaped, and Node also refuses to spawn a .cmd without a shell at all. Both
// doors that carry caller text -- the search query and the install tool name --
// are checked against this before anything spawns, and the command file is
// quoted by quoteForShell, which is the other half the validation misses.
export const QUERY = /^[@A-Za-z0-9][A-Za-z0-9._+@/-]{0,79}$/;

// Node concatenates rather than escapes when shell is true, and that applies to
// the command file as much as to its arguments: a manager under
// C:\Program Files reached cmd.exe as two tokens and reported a search that ran
// and found nothing, when nothing ran (pass 21 measured it). Quoting the path
// is the half of DEP0190 that validating the query does not cover.
export function quoteForShell(p) {
  return process.platform === 'win32' && /\s/.test(String(p)) ? `"${p}"` : p;
}
export function have(binary, runner = null, resolver = resolveBinary) {
  const run = runner || ((b) => {
    ...
    return spawnSync(needShell ? quoteForShell(path || b) : (path || b), ['--version'], { encoding: 'utf8', timeout: 20000, stdio: ['ignore', 'pipe', 'pipe'], shell: needShell });
  });
  ...
  // Present means the binary answered: a resolved path that ran, or a shell
  // probe that came back with something to say. cmd.exe answers an unknown
  // name with exit 1, no error and empty output, which the old test read as
  // present, so every name on win32 looked installed (pass 12).
  try {
    const r = run(binary);
    if (!r || r.error || r.status === null) return false;
    // An injected runner means an injected machine. This line consulted the
    // real filesystem whatever the runner said, so a control could not simulate
    // an absent bun on a machine that has bun, and the absence half of the
    // no-search control passed here while failing on CI (pass 27).
    if (resolver(binary)) return true;
    // What separates the two is output, not the exit code. A real manager that
    // exits 2 on --version still says something (pass 5); cmd.exe answers an
    // unknown name with exit 1, an empty stdout and its complaint on stderr
    // (pass 12). Testing status === 0 AND output failed the first; testing the
    // status alone failed the second.
    return r.status === 0 || String(r.stdout || '').trim().length > 0;
  } catch { return false; }
}

export function search(query, { c = contract(), only = null, runner = null, resolver = resolveBinary } = {}) {
  ...
  // A shell on Windows, because Node cannot spawn a .cmd without one, over a
  // query QUERY has already validated. The resolved path is preferred where
  // there is one (passes 3, 5 and 7; the comment said otherwise until 13).
  ...
  const run = runner || ((m, args, secs) => {
    ...
    const needShell = process.platform === 'win32';
    if (!path && !needShell) return { status: null, error: { code: 'ENOENT' } };
    return spawnSync(needShell ? quoteForShell(path || m.binary) : (path || m.binary), args, { encoding: 'utf8', timeout: secs * 1000, stdio: ['ignore', 'pipe', 'pipe'], shell: needShell });
  });
  const out = [];
  for (const m of c.managers) {
    if (only && !only.includes(m.name)) continue;
    // Named resolver, not resolve: node:path's resolve is imported at the top of
    // this file and returns a truthy absolute path for any string, so shadowing
    // it here would have made every manager read present (pass 27).
    const present = have(m.binary, runner ? (b) => runner({ binary: b }, ['--version'], 20) : null, resolver);
    // A manager that installs but cannot search a registry reports that, rather
    // than reporting an absence it never measured (pass 14). Whether it can
    // search is a property of its declaration, not of this machine, so it is
    // answered before presence and carries the measured presence beside it:
    // asked presence-first, bun and uv came back a plain absent wherever they
    // are not installed, and the control that asserts otherwise passed only on
    // a machine that happens to have all six (pass 27, caught by CI on Linux).
    if (m.search === '-') { out.push({ manager: m.name, absent: !present, noSearch: true, hits: [], exit: null }); continue; }
    if (!present) { out.push({ manager: m.name, absent: true, hits: [], exit: null }); continue; }
...
// The black lists as this module needs them: four files and a regex, read
// directly rather than through list.mjs, because importing that module from
// this one closes a cycle with its own top-level await (pass 9).
export function blackLists(root) {
  const out = { 'file-black': [], 'code-black': [] };
  for (const scope of ['file', 'code']) {
    for (const layer of [join(os.homedir(), '.claude', 'rot-dtd-commander'), root]) {
      const path = join(layer, '.rot-lists', `${scope}-black.dtd`);
      if (!existsSync(path)) continue;
      for (const m of readFileSync(path, 'utf8').matchAll(/<!ENTITY LIST\.entry\.([A-Za-z0-9_-]+)\s+"([^"]*)">/g)) {
        out[`${scope}-black`].push({ name: m[1], reason: (m[2].split('|')[0] || '').trim(), scope, layer: layer === root ? 'repository' : 'machine' });
      }
    }
  }
  return out;
...
export function installPlan(tool, managerName, { c = contract(), lists = null, root = process.cwd() } = {}) {
  // Defaulting to no lists made the black-list refusal fail open on the one
  // path that mutates this machine: installPlan('exe', 'scoop', {}) planned an
  // install of a filetype this repository blacklists (pass 9).
  if (lists === null) {
    try { lists = blackLists(root); } catch { lists = {}; }
  }
  // The same boundary search() has, on the path that actually changes the
  // machine. Pass 7 built "scoop install msys2 & echo ROT_INJECTION_MARKER"
  // through this function while QUERY guarded only the search.
  if (!QUERY.test(String(tool || ''))) {
    return { refused: [`REFUSED install ${JSON.stringify(String(tool)).slice(0, 60)}`,
      '  the name is not one a package name may take: letters, digits and . _ + @ / -',
      '  a shell runs the install on Windows, so anything it reads as syntax is refused before the confirmation is offered',
      '  to resolve: name the package alone'].join('\n'), line: null };
  }
...
export function measure(root, { c = contract() } = {}) {
  ...
  const walk = (dir, depth = 0) => {
    ...
    for (const it of items) {
      // Only the noise is skipped. Skipping every dot-directory hid
      // .github/workflows entirely, so yml was whitelisted and uncounted, and
      // the fourth clause of LAW.LIST.4 could not see it (pass 7).
      if (it.name === 'node_modules' || it.name === 'target' || it.name === '.git' || it.name === '.claude') continue;
  ...
  // What the repository contains, asked of git where there is one: a walk of
  // the filesystem counts local state nobody committed (.cocoindex_code held
  // .mdb and .db files that the guard then refused as though this project
  // shipped them, pass 7). The walk stays as the answer for a tree with no git.
  const tracked = spawnSync('git', ['-C', root, 'ls-files'], { encoding: 'utf8', timeout: 60000, stdio: ['ignore', 'pipe', 'pipe'] });
  let source = 'git';
  if (tracked.status === 0 && String(tracked.stdout || '').trim()) {
    for (const rel of String(tracked.stdout).split(/\r?\n/).map((x) => x.trim()).filter(Boolean)) {
      const base = rel.split('/').pop();
      if (BUILD.has(base)) builds.push(base);
  ...
  return { languages, census, builds: [...new Set(builds)], toolchain, source };
}
...
export function writeSession(root, s, c = contract()) {
  ...
  const lines = [...SPDX, '', '# The starlist session, resumable across blocks', '',
    `- questions per round: ${c.perRound}`,
    `- block: ${s.block}`, `- rounds: ${s.rounds}`, `- measured: ${s.measured || ''}`, '',
    '| block | round | header | answer |', '|---|---|---|---|',
    ...(s.rows || []).map((r) => `| ${r.block} | ${r.round} | ${String(r.header).replace(/\|/g, '/')} | ${String(r.answer).replace(/\|/g, '/')} |`), ''];
...
// STAR.block declares how many rounds a block holds. It was parsed into the
// contract and read by nothing, so a block closed whenever a caller said so
// rather than when the declaration said it was full (pass 21).
export function blockFull(s, c = contract()) { return (s.rounds || 0) >= c.block; }

export function nextBlock(s, settled, c = contract()) {
  if (settled) return { ...s, done: true };
  return { ...s, block: s.block + 1, rounds: 0, done: false, full: false, carried: (s.rows || []).length, of: c.block };
}
...
export function controls(io = console) {
  ...
  const admitted = ['a | b', 'a; b', '`x`', '$(x)', 'a && b', ''].filter((bad) => QUERY.test(bad));
  say(admitted.length === 0, `every shape the pattern must refuse is refused: ${admitted.length === 0 ? '6 of 6' : `admitted ${JSON.stringify(admitted)}`}`);
  ...
  say(walked.census.some(([e]) => e === 'ts'), 'a file deep in the tree is measured: ts from src/skills/*/templates/');
  // measure() takes the git branch here, so the walk and its depth ceiling were
  // named by a control that never ran them (pass 15). A tree with no git does.
  const deep = mkdtempSync(join(os.tmpdir(), 'rot-walk-'));
  const nest = join(deep, 'a', 'b', 'c', 'd', 'e', 'f');
  mkdirSync(nest, { recursive: true });
  writeFileSync(join(nest, 'deep.rs'), 'fn main() {}', 'utf8');
  writeFileSync(join(deep, 'Cargo.toml'), '[package]', 'utf8');
  const walkedDeep = measure(deep);
  say(walkedDeep.source === 'walk' && walkedDeep.census.some(([e]) => e === 'rs') && walkedDeep.builds.includes('Cargo.toml'),
    `the filesystem walk itself is exercised, six directories down: source=${walkedDeep.source}, ${walkedDeep.census.map(([e, n]) => e + ' ' + n).join(', ') || 'nothing'}`);
  say(walked.source === 'git' && walked.census.some(([e]) => e === 'yml') && !walked.census.some(([e]) => e === 'mdb'),
    `the census is what the repository contains, not what sits under the path: source=${walked.source}, yml counted, local index state not`);
  const nowhere = measure(mkdtempSync(join(os.tmpdir(), 'rot-nogit-')));
  say(nowhere.source === 'walk', `a tree with no git falls back to the walk rather than measuring nothing: source=${nowhere.source}`);
  const noSearch = search('ripgrep', { c, only: ['bun', 'uv'] });
  say(noSearch.length === 2 && noSearch.every((r) => r.noSearch === true && r.hits.length === 0),
    `a manager that installs but cannot search a registry says so rather than reporting a measured nothing: ${noSearch.map((r) => r.manager).join(', ')} (LAW.STAR.2)`);
  // The same answer on a machine that has neither. Asked presence-first this
  // returned a plain absent, and the control above passed here only because
  // this machine has all six; CI on Linux failed it (pass 27).
  const goneRunner = () => ({ status: 1, stdout: '', stderr: '' });
  const noSearchGone = search('ripgrep', { c, only: ['bun', 'uv'], runner: goneRunner, resolver: () => null });
  say(noSearchGone.length === 2 && noSearchGone.every((r) => r.noSearch === true && r.absent === true && r.hits.length === 0),
    `cannot-search is a property of the declaration, not of the machine: absent bun and uv still report noSearch, with the absence beside it: ${noSearchGone.map((r) => `${r.manager} absent=${r.absent} noSearch=${r.noSearch}`).join(', ')} (LAW.STAR.2)`);
  say(c.managers.filter((m) => m.search !== '-').length === 4,
    `the four that can search still do: ${c.managers.filter((m) => m.search !== '-').map((m) => m.name).join(', ')}`);
  const sessTmp = mkdtempSync(join(os.tmpdir(), 'rot-sess-'));
  writeSession(sessTmp, { block: 1, rounds: 2, measured: 'rs 2', rows: [] }, c);
  say(readFileSync(sessionPath(sessTmp, c), 'utf8').includes(`questions per round: ${c.perRound}`),
    `the session record carries the declared STAR.per_round rather than dropping it: ${c.perRound}`);
  // LAW.STAR.1 says a seventh manager is one more entity and no new code.
  // The names appear twice in the subset, and nothing compared them until
  // pass 20: this is what makes the second copy safe.
  const dtdText = readFileSync(join(ROOT, 'dtd', 'cc-starlist.dtd'), 'utf8');
  const enumNames = (/manager\s+\(([^)]*)\)/.exec(dtdText) || [null, ''])[1].split('|').filter(Boolean);
  say(enumNames.length === c.managers.length && enumNames.every((n, i) => n === c.managers[i].name),
    `the two spellings of the six agree: STAR.managers ${c.managers.map((m) => m.name).join('|')} and the adopted enumeration ${enumNames.join('|')}`);
  say(!blockFull({ rounds: c.block - 1 }, c) && blockFull({ rounds: c.block }, c),
    `a block is full at the declared STAR.block and not before: ${c.block} rounds`);
  say(nextBlock({ block: 1, rounds: c.block, rows: [1, 2] }, false, c).of === c.block,
    `the block that opens next carries the declared size rather than an assumed one: of ${c.block}`);
  ...
  let leakedInstall = null;
  for (const bad of dangerous) {
    const plan = installPlan(bad, 'scoop', { c });
    if (!plan.refused || plan.line) { leakedInstall = bad; break; }
  }
  say(leakedInstall === null, `the install path refuses the same shapes the search path does: ${dangerous.length} tried, ${leakedInstall === null ? 'none built a line' : `${JSON.stringify(leakedInstall)} did`}`);
  say(installPlan('msys2', 'scoop', { c }).line === 'scoop install msys2', `an ordinary install still plans: ${installPlan('msys2', 'scoop', { c }).line}`);
  ...
  // The real spawn, on a name nobody has: a fake runner cannot see the win32
  // branch that made every name look present (pass 12).
  say(!have('zzznosuchmgr-rot-dtd'), `a name this machine does not have is absent, measured through the real spawn: ${have('zzznosuchmgr-rot-dtd') ? 'REPORTED PRESENT' : 'absent'} (LAW.STAR.2)`);
  say(have('node'), `a binary this machine does have is present, measured the same way: node ${have('node') ? 'present' : 'ABSENT'}`);
  // A resolved binary is present whatever it exits with; an unresolved name
  // must at least answer with something, or a shell that says "not recognized"
  // would read as present (pass 12).
  say(have('node', () => ({ status: 2, stdout: '' })) && !have('x', () => ({ status: 1, stdout: '' })) && !have('x', () => ({ status: null, error: { code: 'ENOENT' } })),
    'a manager that answers with any exit code is present; one that cannot be run is absent (LAW.STAR.2)');
  const byType = installPlan('bench.exe', 'scoop', { c, lists: { 'file-black': [{ name: 'exe', reason: 'nothing here ships a binary', layer: 'repository', scope: 'file' }] } });
  say(Boolean(byType.refused) && /exe/.test(byType.refused) && !byType.line,
    `the filetype half of the refusal fires too, not only the name: ${String(byType.refused).split('\n')[0].slice(0, 60)}`);
  const openByDefault = installPlan('exe', 'scoop', { c, root: ROOT });
  say(Boolean(openByDefault.refused), `an install with no lists passed reads the repository rather than failing open: ${openByDefault.refused ? 'refused' : 'PLANNED ' + openByDefault.line}`);
  ...
  // The old predicate asserted a condition and its negation, so it could
  // only go red on a state that cannot occur (pass 18). What it should say:
  // a manager that is present ran, and one that is absent said so.
  say(real.length === 1 && (real[0].absent === true || (real[0].couldNotRun !== true && real[0].exit !== 127)),
...
if (isMain) {
  ...
  if (cmd === 'install') {
    // LAW.STAR.3's enforcement had no route from any command: installPlan was
    // exported and called only by controls, so /starlist-dtd could describe a
    // refusal it had no way to obtain (pass 21). This verb is that route, and
    // it prints the plan rather than running it.
    const [tool, manager] = rest;
    if (!tool || !manager) { console.log('usage: node lib/starlist.mjs install <tool> <manager>'); process.exit(2); }
    const plan = installPlan(tool, manager, { c, root: process.cwd() });
    if (plan.refused) { console.log(plan.refused); process.exit(1); }
    for (const line of plan.shows) console.log(line);
    console.log('confirm before this runs: it is printed, never spawned by this verb (LAW.STAR.3)');
    process.exit(0);
  }
  ...
  if (cmd === 'search') {
    ...
    for (const r of search(rest.join(' '), { c })) console.log(r.absent ? `${r.manager}: not installed on this machine` : r.noSearch ? `${r.manager}: installs but does not search a registry` : `${r.manager}: exit ${r.exit}${r.timedOut ? ' (ceiling)' : ''}, ${r.hits.length} hits${r.hits.length ? ': ' + r.hits.slice(0, 3).join(' | ') : ''}`);
    process.exit(0);
  }
