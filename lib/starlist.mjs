...
export const SHELL_PROBE = process.platform === 'win32';
export function have(binary, runner = null) {
  const run = runner || ((b) => spawnSync(b, ['--version'], { encoding: 'utf8', timeout: 20000, stdio: ['ignore', 'pipe', 'pipe'], shell: SHELL_PROBE }));
...
export function search(query, { c = contract(), only = null, runner = null } = {}) {
  // The same shell have() needs: a .cmd shim cannot be spawned without it, and
  // searching every present manager returned ENOENT (third companion pass).
  const run = runner || ((m, args, secs) => spawnSync(m.binary, args, { encoding: 'utf8', timeout: secs * 1000, stdio: ['ignore', 'pipe', 'pipe'], shell: SHELL_PROBE }));
  const out = [];
