...
export function have(binary, runner = null) {
  ...
  try {
    ...
    if (resolveBinary(binary)) return true;
...
export function search(query, { c = contract(), only = null, runner = null } = {}) {
  ...
  for (const m of c.managers) {
    ...
    // than reporting an absence it never measured (pass 14).
    if (m.search === '-') { out.push({ manager: m.name, absent: false, noSearch: true, hits: [], exit: null }); continue; }
