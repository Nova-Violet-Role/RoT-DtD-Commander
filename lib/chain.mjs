...
export function resolve(key, root = ROOT) {
  ...
  // A command is runnable alone when it declares its own root and does not
  // require an artifact it cannot have been given (LAW.CHAIN.2).
  const needsArtifact = /(survey|plan)\s+CDATA\s+#REQUIRED/.test(t);
  return { key, exists: true, root: rootEl, hands_to: hands, band, needsArtifact, bytes: t.length };
}

// ---- the plan (every law checked before link one runs) ----
export function plan(text, { root = ROOT } = {}) {
  const raw = read(text);
  ...
  raw.slice(0, MAX).forEach((r, i) => {
    ...
    if (i > 0) {
      handoffs.push({ from: raw[i - 1].token, to: r.token, artifact: `${DIR}/${raw[i - 1].key}.md`, as: 'user-args' });
    }
    links.push(link);
  });
...
export function render(p) {
  ...
  out.push(`  <chain_intake rounds="0" questions="0" asked="${p.autonomy === 'no-gate' ? 'no' : 'yes'}"${p.autonomy === 'no-gate' ? ` reason="${esc(TOKEN)} was present"` : ''}/>`);
  ...
  for (const h of p.handoffs) {
    out.push(`  <handoff from="${esc(h.from)}" to="${esc(h.to)}" artifact="${esc(h.artifact)}" bytes="0" as="user-args" trust="cdata"/>`);
  }
...
function main() {
  ...
  if (args[0] === 'plan') {
    const p = plan(args.slice(1).join(' ').split('\\n').join('\n'));
    console.log(render(p));
    for (const r of p.refusals) console.log(`  REFUSED ${r.why}: ${r.text}`);
    process.exit(p.ok ? 0 : 1);
  }
  console.log('usage: node lib/chain.mjs plan "<stacked prompt>" | check | controls');
}
