...
export function expectedFromCommand(text) {
  ...
  // A required element the grammar_map omits was never in this list, so the
  // missing_heading rule could not fire for it -- the one case where a command
  // and its own DOCTYPE disagree (pass 22). The content model is the authority
  // on what is required; the map is the authority on how it renders.
  const headings = [];
  const requiredByModel = new Set(required || []);
  let sigil = null;
  if (gm) {
  ...
  // A required element the grammar_map never names: the rule that reports a
  // missing heading walked only what the map declared, so a command whose own
  // content model required something its map omitted passed every check (found
  // by pass 22 of the 7.0.0 audit). The model is the authority on what is
  // required; the map is the authority on how it renders.
  for (const name of requiredByModel) {
    if (!headings.some((h) => h.element === name)) {
      headings.push({ element: name, heading: null, required: true, unmapped: true });
    }
  }
