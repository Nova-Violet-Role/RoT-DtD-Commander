...
//
//   expectedFromCommand(text)          -> { root, laws, headings[], required[], unmappedRequired[], children[], sigil, hasRefs, autonomousAware }
...
export function expectedFromCommand(text) {
  ...
  if (gm) {
    for (const line of gm[1].split('\n')) {
      ...
      if (!row) continue;
      const el = row[1];
      // A row whose heading carries no bold was dropped in silence, and a
      // command written that way lost every row it had (pass 24 measured six of
      // six dropped in whats-next-dtd). The bold is a convention, not the
      // grammar: the text after the colon is the heading either way.
      let bolds = [...row[2].matchAll(/\*\*([^*]+)\*\*/g)].map((m) => stripMd(m[1]));
      if (!bolds.length) {
        const plainRow = stripMd(row[2].split(',')[0].trim());
        if (plainRow) bolds = [plainRow];
      }
  ...
  // A command that includes cc-cache is held to the gate law at Stop: its
  // rendered intake may not close on a re-entry, and a gate answered save
  // renders the cache element (LAW.CACHE.2, LAW.CACHE.5, control C31).
  const cache = sub.entities.has('LAW.CACHE.1');
  // A required element the grammar_map never names is a disagreement between a
  // command and its own DOCTYPE. It is reported here as its own field, never
  // pushed into headings: pass 22 pushed one with a null heading and every
  // consumer that calls stripSigil threw, which the observe verb caught and
  // exited 0 on -- the Stop gate failed open for fifteen commands (pass 23).
  const unmappedRequired = [...required].filter((n) => !headings.some((h) => h.element === n));
  return { root: d.name, laws, headings, required: [...required], unmappedRequired, hasRefs, autonomousAware, children, sigil, cache };
}

// The last gate line of a rendered intake (`- gate: start (round 2)`,
// `**Gate:** \`save\``), and whether a cache line names a .nt file.
export function gateOfAnswer(lines) {
  let last = null;
  let cacheLine = false;
  for (const l of lines) {
    const m = /^\s*(?:[-*]\s*|\d+\.\s*)?(?:\*\*)?gate(?:\*\*)?\s*:\**\s*`?(start|more|add|impactful|save)\b/i.exec(l);
    if (m) last = m[1].toLowerCase();
    if (/^\s*(?:[-*]\s*)?(?:\*\*)?cache(?:\*\*)?\s*:\**.*\.nt\b/i.test(l)) cacheLine = true;
  }
  return { last, cacheLine };
}
...
export function checkAnswer(answer, expected, { autonomous = false } = {}) {
  ...
  // The gate law of cc-cache (control C31): an intake that closed on add,
  // more or impactful never presented its gate again (LAW.CACHE.5), and a
  // gate answered save without a cache line saved nothing (LAW.CACHE.2). A
  // command that does not include the subset is asked nothing here.
  if (expected.cache) {
    const g = gateOfAnswer(from > 0 ? lines.slice(from) : lines);
    if (g.last && ['add', 'more', 'impactful'].includes(g.last)) {
      findings.push({ kind: 'gate', msg: `intake closed on the re-entry ${g.last}: the gate is presented again after every add, more or impactful (LAW.CACHE.5)` });
    }
    if (g.last === 'save' && !g.cacheLine) {
      findings.push({ kind: 'cache', msg: 'gate answered save without a cache line naming the .nt file written and read back (LAW.CACHE.2)' });
    }
  }
...
export function prescribe(findings, expected, command) {
  ...
  const slop = findings.filter((f) => f.kind === 'slop');
  const charmSlop = slop.length ? ` The AI_SLOP gate failed on ${slop.map((f) => f.msg.replace(/^slop: /, '')).join('; ')}: rewrite the answer's own voice with active verbs, moving sentence length and no ban-list phrase (LAW.SLOP.1 to 6), then re-run.` : '';
  const gate = findings.filter((f) => f.kind === 'gate' || f.kind === 'cache');
  const charmGate = gate.length ? ' The gate law failed: present the gate again after every add, more or impactful (LAW.CACHE.5), and on save write the cache with node lib/cache.mjs save, render the cache line with the file and its bytes, and end the answer with CACHE.compact (LAW.CACHE.2).' : '';
  ...
  return { charm: charm + charmSlop + charmGate, rite };
}
