...
// check         the both-direction contract check (rules C1..C14)
...
export function check(resolvedText, { includes = {} } = {}) {
  ...
  if (gmBlock) {
    const mapped = new Set([...gmBlock[1].matchAll(/^-\s+`([\w.:-]+)`:/gm)].map((m) => m[1]));
    const optional = /[?*]$/;
    for (const raw of String(rootModel).split(/[,|()]/)) {
      const name = raw.trim();
      if (!name || name === "#PCDATA" || optional.test(name)) continue;
