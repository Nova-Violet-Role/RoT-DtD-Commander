...
const TABLE_PATH = join(ROOT, 'src', 'skills', 'iupac-ordinals-dtd', 'references', 'greek-cardinals.md');
...
export function controls(io = console) {
  ...
  if (existsSync(join(ROOT, 'src'))) {
    if (!existsSync(TABLE_PATH)) fail.push('references/greek-cardinals.md is missing: run node lib/ordinals.mjs table and write it to ' + TABLE_PATH);
    else if (readFileSync(TABLE_PATH, 'utf8').split(String.fromCharCode(13)).join('') !== table()) fail.push('references/greek-cardinals.md drifted: run node lib/ordinals.mjs table and write it to ' + TABLE_PATH);
