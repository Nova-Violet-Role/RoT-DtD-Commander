...
export function readState(root, c = contract()) {
  ...
  const empty = { run: 0, verb: 0, offset: 0, page: 0, from: '', release: '', walked: [], rows: [] };
  if (!existsSync(p)) return empty;
  const text = readFileSync(p, 'utf8');
  const head = {};
  for (const m of text.matchAll(/^- (run|verb|offset|release|walked|page|from): (.*)$/gm)) head[m[1]] = m[2].trim();
  const rows = [];
  // Column 8 is run, appended in 7.1.0 (LAW.REC.2). A row written before it
  // has seven columns and reads as run 0, which is older than any real run,
  // so an old record still parses and never counts as the current release.
  for (const m of text.matchAll(/^\| ([0-9a-f]{8}) \| (gap|idea) \| ([a-z]+) \| (exposed|marked|refused|done|reopen) \| ([0-9]+) \| ([0-9]*) \| ([^|]*?) \|(?: ([0-9]*) \|)?$/gm)) {
    rows.push({ id: m[1], class: m[2], layer: m[3], verdict: m[4], verb: Number(m[5]), refused_at: Number(m[6] || 0), why: m[7].trim(), run: Number(m[8] || 0) });
  }
  return { run: Number(head.run || 0), verb: Number(head.verb || 0), offset: Number(head.offset || 0), page: Number(head.page || 0), from: head.from || '', release: head.release || '', walked: (head.walked || '').split(/[, ]+/).filter(Boolean), rows };
}
...
export function writeState(root, state, c = contract()) {
  ...
  const lines = [...SPDX, STATE_HEAD, '', '# amplify-codebase: the state between runs', '',
    `- run: ${state.run}`, `- verb: ${state.verb}`, `- offset: ${state.offset}`, `- page: ${state.page || 0}`, `- from: ${state.from || ''}`, `- release: ${state.release || ''}`, `- walked: ${(state.walked || []).join(', ')}`,
    '', '| id | class | layer | verdict | verb | refused_at | why | run |', '|---|---|---|---|---|---|---|---|'];
  // A why is written whole: the 160-character cap cut a8f0de10 mid-word and
  // the ledger re-emitted from the record lost twenty-nine characters of
  // committed text, found by the third companion pass. A pipe is the one
  // character replaced, because it is the column separator.
  for (const r of state.rows) lines.push(`| ${r.id} | ${r.class} | ${r.layer} | ${r.verdict} | ${r.verb} | ${r.refused_at || ''} | ${String(r.why).replace(/\|/g, '/').replace(/\r?\n/g, ' ')} | ${r.run || state.run} |`);
  lines.push('');
  const p = statePath(root, c);
...
export function controls(io = console) {
  ...
  // LAW.AMP.3: the two classes are separated by the code, not only by the
  // prose. A gap must carry an instrument, an idea must carry what it adds,
  // and every gap outranks every idea whatever their risk.
  const classed = [
    { id: 'ddddddd1', class: 'gap', risk: 'low', files: ['x'], verb: 2, evidence: { instrument: 'node checker/contract-audit.mjs' }, confidence: 'measured' },
    { id: 'ddddddd2', class: 'idea', risk: 'high', files: ['x'], verb: 9, evidence: { adds: 'LAW.DIAG.1' }, confidence: 'reasoned' },
  ];
  const order = rank(classed).map((p) => p.class);
  say(order.join(',') === 'gap,idea'
    && classed.every((p) => (p.class === 'gap' ? !!p.evidence.instrument && p.confidence === 'measured' : !!p.evidence.adds && p.confidence !== 'measured')),
    `every gap outranks every idea whatever the risk, a gap names an instrument and an idea names what it adds: ${order.join(' then ')} (LAW.AMP.3)`);
  ...
  const written = writeState(tmp, { run: 3, verb: 8, offset: 41, from: '5.1.0', release: 'mid 5.2.0', walked: ['schematic', 'record'], rows: [{ id: 'a3f10000', class: 'gap', layer: 'record', verdict: 'refused', verb: 7, why: 'six commands declare a record they never write' }] });
  ...
  say(back.from === '5.1.0', `the state record carries the version the release is measured from, which the version gate reads: from ${back.from || 'MISSING'} (LAW.AMP.14)`);
  // A long why round-trips whole: the record used to cap it at 160 characters
  // and the study re-emitted from the record lost committed text (LAW.AMP.6).
  const longWhy = 'w'.repeat(120) + ' | the pipe becomes a slash | ' + 'x'.repeat(120);
  writeState(tmp, { run: 3, verb: 8, offset: 0, from: '5.1.0', walked: ['record'], rows: [{ id: 'a3f10001', class: 'gap', layer: 'record', verdict: 'done', verb: 2, why: longWhy }] });
  const longBack = readState(tmp).rows[0];
  say(longBack && longBack.why.length === longWhy.length && longBack.why.startsWith('w'.repeat(120)) && longBack.why.endsWith('x'.repeat(120)) && !longBack.why.includes('|'),
    `trip: a why of ${longWhy.length} characters round-trips whole through the record, ${longBack ? longBack.why.length : 0} back, the pipe replaced and nothing cut`);
  ...
  // LAW.AMP.6 must defer to LAW.AMP.12 rather than contradict it: the code
  // re-offers a refusal whose runs are up, so the law may not say never.
  const six = c.lawText.get('LAW.AMP.6') || '';
  const twelve = c.lawText.get('LAW.AMP.12') || '';
  say(!/never offered again/.test(six) && /LAW\.AMP\.12/.test(six) && /reopen/.test(twelve),
    'LAW.AMP.6 defers to LAW.AMP.12 on when a refusal returns, which is what unrefused() implements');
  ...
  // The audit of v6.0.0 found the raised rounds never bound: predeclare takes
  // entity BODIES and the spec passed whole declarations, which the forge
  // wrapped again. Read the built command, not the spec that claims it.
  const built = join(ROOT, 'commands', 'amplify-codebase-dtd.md');
  if (existsSync(built)) {
    const text = readFileSync(built, 'utf8');
    const doctype = text.slice(0, text.indexOf(']>') + 2);
    const doubled = /<!ENTITY\s+<!ENTITY/.test(text);
    const rounds = (/<!ENTITY % ask\.rounds "\(([^)]*)\)">/.exec(doctype) || [])[1] || '';
    const perPrompt = (/<!ENTITY ASK\.rounds_per_prompt "([0-9]+)">/.exec(doctype) || [])[1] || '';
    const total = (/<!ENTITY ASK\.max_total\s+"([0-9]+)">/.exec(doctype) || [])[1] || '';
    say(!doubled && rounds === '1|2|3|4|5' && perPrompt === '5' && total === '20',
      `the built command binds the raised rounds it claims: ask.rounds (${rounds}), per prompt ${perPrompt}, total ${total}, doubled declarations ${doubled} (LAW.ASK.11)`);
    // Derived from the declarations, never repeated: the control must fail
    // when the DTD and the built command disagree, not when a literal here does.
    const bands = Object.fromEntries(Object.entries(c.bands).map(([k, ns]) => [`${k}-codebase-dtd.md`, `${Math.min(...ns)}-${Math.max(...ns)}`]));
    let pinned = 0;
    for (const [file, band] of Object.entries(bands)) {
      const p = join(ROOT, 'commands', file);
      if (existsSync(p) && new RegExp(`band\\s+CDATA #FIXED "${band}"`).test(readFileSync(p, 'utf8'))) pinned++;
    }
    say(pinned === 3, `every command of the family pins its band as a #FIXED attribute: ${pinned} of 3 (LAW.AMP.4)`);
  } else say(false, 'the built command is missing: run npm run build');

  io.log(`amplify controls: ${ran} run, ${fail} failing`);
  return fail === 0;
}
...
if (isMain) {
  ...
  if (cmd === 'study') {
    // The run's own data as JSON: { run, date, page, offset, verb, layers[], possibilities[], release }.
    // Written literally, so a finding carrying a dollar sign or a backtick survives (LAW.AMP.13).
    const src = rest[0];
    if (!src || !existsSync(src)) { console.log('usage: node lib/amplify.mjs study <run.json>'); process.exit(2); }
    const runData = JSON.parse(readFileSync(src, 'utf8'));
    const docs = writeStudy(process.cwd(), runData, { c });
    const held = studyHolds(docs);
    for (const d of docs) console.log(`${d.kind} ${d.path}`);
    console.log(`study: ${docs.length} document(s), kinds ${held.holds ? 'complete' : 'MISSING ' + held.missing.join(', ')}`);
    process.exit(held.holds ? 0 : 1);
  }
  ...
  console.log('usage: node lib/amplify.mjs detect | walk [layer...] | state | study <run.json> | recognize [--from=x.y.z] [--stage=alpha|beta|pre] <verb...> | table | controls');
  process.exit(2);
}
