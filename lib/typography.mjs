...
export function controls() {
  ...
  // control reads the table's row rather than remembering the divergence
  // (seventeenth companion pass on 9.0.0).
  const greek = readFileSync(new URL('../dtd/sigil/greek-numbers.md', import.meta.url), 'utf8');
  const row29 = /\|\s*29\s*\|\s*\*\*([a-z]+)-\*\*/.exec(greek);
  say(row29 && numeral(29).name === 'icosiennea' && row29[1] === numeral(29).name,
    `the construction rule is followed at 29 (${numeral(29).name}) and the table's own row agrees (${row29 ? row29[1] : 'no row'})`);
