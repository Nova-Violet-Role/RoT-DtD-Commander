...
//   resolve(ch, { fallback }) -> what is actually drawn for one character; the class switch lives in the grammar (LAW.TYPO.4), not here
...
// The default driver resolved: the enumerations are read from this text,
// since the glyph class enumeration is a parameter entity the modules
// govern and the raw grammar carries only its reference.
const RESOLVED = (() => {
  const all = resolveSubset('<!ENTITY % cc-figure SYSTEM "cc-figure.dtd">\n%cc-figure;\n<!ENTITY % typography SYSTEM "typography.dtd">\n%typography;', join(ROOT, 'dtd')).text;
  // the typography segment alone: the figure grammar it resolves beside has
  // enumerations of its own, with producers of its own
  const m = /<!-- begin subset typography -->([\s\S]*?)<!-- end subset typography -->/.exec(all);
  if (!m) throw new Error('the resolved default driver carries no typography segment');
  return m[1];
})();
...
export function resolve(ch, { fallback = null } = {}) {
  const g = guarantee();
...
export function enumerations(text = RESOLVED) {
  const out = [];
...
export function enumerationCount(text = RESOLVED) {
...
export function controls() {
  ...
  // dtd/sigil/greek-numbers.md once wrote 29 as "icosienna" against its own
  // rule, icosi + ennea; the table now agrees with the constructor, and this
  // control reads every row of the table the constructor names rather than
  // one row or a memory of the divergence (seventeenth and eighteenth
  // companion passes on 9.0.0).
  const greek = readFileSync(new URL('../dtd/sigil/greek-numbers.md', import.meta.url), 'utf8');
  const rows = [...greek.matchAll(/^\|\s*(\d+)\s*\|\s*\*\*([a-z]+)-\*\*/gm)].map((m) => [Number(m[1]), m[2]]);
  const constructed = rows.filter(([n]) => numeral(n).form !== 'plain');
  const plainRows = rows.length - constructed.length;
  const disagree = constructed.filter(([n, name]) => numeral(n).name !== name);
  say(rows.length >= 40 && constructed.length >= 30 && disagree.length === 0 && numeral(29).name === 'icosiennea',
    `every row of the table the constructor names agrees with it: ${constructed.length} of ${rows.length} rows compared, ${plainRows} beyond the carve-out named by number (LAW.TYPO.7), ${disagree.length} disagree${disagree.length ? ` (${disagree.map(([n, name]) => `${n}: table ${name}, constructor ${numeral(n).name}`).join('; ')})` : ''}`);
  ...
  say(/never named by us/.test(widget),
    "the widget face is the viewer's own and is never named by us");
  ...
  say(/class\s+\(ascii\|rule\|arrow\|math\|greek\)\s+#REQUIRED/.test(on) && !/\|block\)/.test(on),
    'with the default driver the glyph class enumeration carries the four INCLUDE classes and ascii, never block (LAW.TYPO.4)');
  ...
  const withBlock = resolveSubset('<!ENTITY % typo.block.module "INCLUDE">\n<!ENTITY % typo.classes.content "rule_glyphs?, arrow_glyphs?, math_glyphs?, greek_glyphs?, block_glyphs?">\n<!ENTITY % typo.glyph.classes "ascii|rule|arrow|math|greek|block">\n<!ENTITY % cc-figure SYSTEM "cc-figure.dtd">\n%cc-figure;\n<!ENTITY % typography SYSTEM "typography.dtd">\n%typography;', base).text;
  say(/<!ELEMENT block_glyphs \(glyph\+\)>/.test(withBlock) && /block_glyphs\?, missing_glyph/.test(withBlock) && /class\s+\(ascii\|rule\|arrow\|math\|greek\|block\)/.test(withBlock),
    'a command that wants block switches it on in three declarations before the include and the grammar gains the element and the class');
  ...
  const producers = {
    ...
    class: classes().names.filter((c) => classes().on[c]),
    form: [1, 13, 20, 21, 100, 144].map((n) => numeral(n).form),
    suffix: SUFFIXES().map((s) => numeral(12, s).suffix),
    // Each driver renders the figure fixture and the value is what came out,
    // not a typeof test over the exports (nineteenth companion pass).
    renders: (() => {
      const fig = figureRenders.parse('<figure grid="12x3" title="renders"><rect x="0" y="0" w="4" h="3" name="a"/></figure>');
      const cells = figureRenders.renderCells(fig);
      const svg = figureRenders.renderSvg(fig, 'light');
      return ['cells', 'svg', 'both'].filter((r) => (r === 'cells' && typeof cells === 'string' && cells.length > 0) || (r === 'svg' && /<svg/.test(String(svg))) || (r === 'both' && typeof cells === 'string' && /<svg/.test(String(svg))));
    })(),
  };
  ...
  const plantedDtd = RESOLVED.replace('sans-serif)', 'sans-serif|cursive)');
