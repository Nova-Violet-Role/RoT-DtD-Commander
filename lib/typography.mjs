...
// Source: artifacts/_sweep/byproducts/GREEK_NUMBERS.md. The series is read from
...
// The compound stems. GREEK_NUMBERS.md writes the twenties with the stem
...
export function enumeration(attr) {
  const m = new RegExp('\\b' + attr + '\\s+\\(([^)]+)\\)').exec(DTD);
  if (!m) throw new Error(`typography.dtd declares no enumeration ${attr}`);
  return m[1].split('|').map((s) => s.trim()).filter(Boolean);
}
export function enumerations() {
  const out = [];
  for (const m of DTD.matchAll(/^\s+([a-z_]+)\s+\(([a-z_|]+)\)/gm)) out.push({ attr: m[1], values: m[2].split('|') });
  return out;
}
...
export function face(role, style = 'normal') {
  ...
  const generic = role === 'plate' ? family.split(',').map((s) => s.trim()).pop() : '';
...
export function numeral(n, suffix = 'gon') {
  ...
  // by its number, which is what GREEK_NUMBERS.md's own closing note says.
  const scales = { 100: used('TYPO.numeral.hundreds'), 1000: used('TYPO.numeral.thousands'), 10000: used('TYPO.numeral.myriads') };
  if (scales[N]) return { n: N, name: scales[N], form: 'ten', suffix };
...
export function controls() {
  ...
  const cases = [[1, 'mono', 'unit'], [3, 'tri', 'unit'], [8, 'octa', 'unit'], [12, 'dodeca', 'unit'],
    ...
    [32, 'triacontadi', 'compound'], [90, 'enneaconta', 'ten'], [100, 'hecta', 'ten']];
  ...
  // GREEK_NUMBERS.md is internally inconsistent at 29 (its table writes
  ...
  say(numeral(29).name === 'icosiennea',
    `the construction rule is followed at 29 (${numeral(29).name}); GREEK_NUMBERS.md's own table writes "icosienna" and the two disagree in the source`);
  ...
  const producers = {
    ...
    class: classes().names,
    form: [1, 13, 20, 21, 144].map((n) => numeral(n).form),
  ...
  say(enums.length === 6 && gaps.length === 0,
