...
export function writeList(path, scope, cls, rows) {
  ...
  for (const r of rows) lines.push(`<!ENTITY LIST.entry.${r.name} "${[r.reason, r.date, r.granted || ''].join('|').replace(/"/g, "'")}">`);
  lines.push('');
...
export function reach(lists, { present = [], produces = {} } = {}) {
  const out = [];
...
// that failed.
export function markdown(lists, starlist = []) {
...
export function controls(io = console) {
  ...
  const poisoned = { 'code-black': [{ name: 'cpp', reason: 'no C++ compiles here', layer: 'repository' }], 'code-white': [{ name: 'dll', reason: 'the shipped artifact', layer: 'repository' }], 'file-white': [], 'file-black': [], 'file-gray': [], 'code-gray': [] };
  ...
  const both = { 'file-white': [{ name: 'py', reason: 'scripts', layer: 'repository' }], 'file-black': [{ name: 'py', reason: 'no second runtime', layer: 'machine' }], 'code-white': [], 'code-black': [], 'file-gray': [], 'code-gray': [] };
  const r2 = reach(both);
  ...
  const strict = { 'code-black': [{ name: 'cpp', reason: 'none here', layer: 'repository' }], 'file-white': [{ name: 'cpp', reason: 'headers', layer: 'repository' }], 'code-white': [], 'file-black': [], 'file-gray': [], 'code-gray': [] };
  ...
  const clean = { 'file-white': [{ name: 'mjs', reason: 'the toolchain', layer: 'repository' }], 'file-black': [], 'file-gray': [], 'code-white': [], 'code-black': [], 'code-gray': [] };
...
if (isMain) {
  ...
  if (cmd === 'reach') {
    const present = [...new Set(readdirSync(process.cwd(), { recursive: false }).map((f) => (f.includes('.') ? f.split('.').pop() : null)).filter(Boolean))];
    const out = reach(lists, { present });
