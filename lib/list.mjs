...
export function starlistPath(root, c = contract(), starFile = null) {
  return join(root, c.dir, starFile || starFileName());
}
...
export function writeStarlist(root, rows, c = contract()) {
  ...
  for (const r of rows || []) {
    if (!NAME.test(String(r.name || ''))) throw new Error(`starlist: "${r.name}" is not a name a declaration may carry`);
  }
  const lines = [...SPDX, '',
  ...
  for (const r of rows) lines.push(`<!ENTITY STAR.have.${r.name} "${[fold(r.reason), fold(r.date), fold(r.granted)].join('|')}">`);
  lines.push('');
...
export function readList(path) {
  ...
  for (const m of text.matchAll(/<!ENTITY LIST\.entry\.([A-Za-z0-9_-]+)\s+"([^"]*)">/g)) {
    const [reason, date, granted, madeBy] = m[2].split('|');
    rows.push({ name: m[1], reason: (reason || '').trim(), date: (date || '').trim(), granted: (granted || '').trim() || null, madeBy: (madeBy || '').trim() || null });
  }
  return rows;
}
...
export function writeList(path, scope, cls, rows, c = contract()) {
  ...
  for (const r of rows) lines.push(`<!ENTITY LIST.entry.${r.name} "${[fold(r.reason), fold(r.date), fold(r.granted), fold(r.madeBy)].join('|')}">`);
  lines.push('');
  writeFileSync(path, lines.join('\n'), 'utf8');
  return { path, count: rows.length };
}
...
if (isMain) await (async () => {
  ...
  if (cmd === 'reach') {
    ...
    process.exit(out.length === 0 && files.stray.length === 0 ? 0 : 1);
  }
  if (cmd === 'md') {
