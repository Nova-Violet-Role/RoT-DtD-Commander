...
export function writeStarlist(root, rows, c = contract()) {
  ...
  for (const r of rows || []) {
    if (!NAME.test(String(r.name || ''))) {
      throw new Error(refusal({ asked: `record ${JSON.stringify(String(r.name)).slice(0, 60)} in the starlist`, entry: 'the name', cls: 'starlist', scope: 'star', layer: 'repository',
...
export function controls(io = console) {
  ...
  const offered = grayAsk({ name: 'ps1', reason: 'outside the ceiling rules', layer: 'repository' }, { 'file-white': [MD, { name: 'sh', reason: 'the audit', layer: 'repository' }], 'code-white': [] }, c);
...
if (isMain) await (async () => {
  ...
  if (cmd === 'reach') {
    ...
    const starlist = [...starlistOf(process.cwd(), repoOnly ? { c, machine: join(process.cwd(), '.rot-lists-no-machine-layer') } : { c }).map((r) => r.name), ...walked.toolchain];
  ...
  if (cmd === 'md') {
    ...
    const m = markdown(lists, [...starlistOf(process.cwd(), { c }).map((r) => r.name)], c);
