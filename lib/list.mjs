...
// Percent and ampersand open a parameter-entity and a general-entity
// reference inside an ENTITY value, so a reason carrying %x; or &x; wrote a
// declaration that refers to something rather than saying it (pass 19).
...
export function writeStarlist(root, rows, c = contract()) {
  ...
  const lines = [...SPDX, '',
    ...
    // The model cc-starlist.dtd declares for this element, not a second
    // incompatible one: a starlist holds tool elements (pass 18).
    '<!ELEMENT tools (tool*)>',
    '<!ELEMENT tool (#PCDATA)>',
    '<!ATTLIST tool',
    '          name      CDATA #REQUIRED',
...
export function reach(lists, { present = [], produces = {}, starlist = null } = {}) {
  ...
  // LAW.LIST.4's third clause, which LAW.STAR.6 rests on: a class promised
...
export const FAMILY = [
  ...
  // One axis: the starlist pins a kind where the six pin a scope and a class.
  { name: 'starlist', scope: null, cls: null, kind: 'starlist' },
...
export function familyHolds(root = ROOT, c = contract()) {
  ...
  for (const f of FAMILY) {
    ...
    // The starlist has one axis and pins a kind where the six pin a scope and
    // a class (pass 19).
    if (f.kind && !new RegExp(`kind\\s+CDATA #FIXED "${f.kind}"`).test(text)) out.push(`${f.name}: does not pin its kind as ${f.kind}`);
...
export function controls(io = console) {
  ...
  // The third clause of LAW.LIST.4 (first companion pass of 7.0.0).
...
if (isMain) await (async () => {
  ...
  if (cmd === 'reach') {
    ...
    // it as reachable in the third clause of LAW.LIST.4 (pass 14).
