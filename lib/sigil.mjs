...
export function contract(path = DTD) {
  ...
  return {
    ...
    lawsCount: Number(ent('SIGIL.laws.count')),
  };
}
...
// below is an older expectation, { bash, expect }: a leg whose bash is older
// than the version named is held to that one instead, and the run says so
// beside the trial (expectation below).
export const TRIALS = [
  ...
  // a subshell whose expansion failed: exit 1 on bash 5 (the ubuntu and windows
  // legs), 127 on bash 3.2 (the macOS leg), measured by the first hosted run of
  // 9.0.0; the line is drawn at 4.0 and bash 4 itself is unmeasured
  { form: 'error-if-unset', script: 'unset x; (echo ${x:?required}) 2>/dev/null; echo $?', expect: '1', below: { bash: '4.0', expect: '127' } },
  ...
  // The two tier B forms the fifth companion pass on 9.0.0 measured declared and never run.
  { form: 'process-substitution-out', script: 't=$(mktemp); echo hi > >(cat > "$t"); sleep 0.3; cat "$t"; rm -f "$t"', expect: 'hi' },
  { form: 'locale-quote', script: 'echo $"hello"', expect: 'hello' },
];
...
// The expectation a leg is held to: the trial's own, or the older one its
// below field names when the leg's bash is older than that version.
export function expectation(t, bash) {
  if (t.below && bash && !vge(bash, t.below.bash)) return { expect: t.below.expect, note: `bash below ${t.below.bash} answers ${t.below.expect}, this leg has ${bash}` };
  return { expect: t.expect, note: '' };
}
export function run(scope = 'all', c = contract()) {
  ...
  const trials = TRIALS.filter((t) => scope === 'all' || tierOf(t.form) === scope || t.form === scope).map((t) => {
    ...
    const ok = e.expect instanceof RegExp ? e.expect.test(actual) : actual === e.expect;
    return { form: t.form, tier: tierOf(t.form), expect: String(e.expect), actual: actual || String(r.stderr || '').trim().slice(0, 80), result: ok ? 'pass' : 'fail', rule: t.rule || '', note: e.note };
  });
...
export function writeRun(root, r, { c = contract(), date = null } = {}) {
  ...
  const md = ['<!-- SPDX-License-Identifier: AGPL-3.0-or-later OR EUPL-1.2 -->', '<!-- Copyright 2026 Saimonokuma. -->', '', `# Sigil run on ${r.leg.os}, bash ${r.leg.bash}, ${d}`, '',
    ...
    ...r.trials.map((t) => `| ${t.form} | ${t.tier} | ${t.expect.replace(/\|/g, '\\|')} | ${String(t.actual).replace(/\|/g, '\\|')} | ${t.result}${t.needs ? ` (${t.needs})` : ''}${t.note ? ` (${t.note})` : ''} |`), '',
...
export function controls(io = console) {
  ...
  // The count comes from SIGIL.laws.count, not from a literal here: this line
  // read `=== 8` against a subset declaring ten and printed "1 to 10" beside
  // it, so the message and the assertion disagreed in the same string.
  const lawsDense = c.laws.length === c.lawsCount && c.laws.every((k, i) => k === `LAW.SIGIL.${i + 1}`);
  say(lawsDense, `LAW.SIGIL.1 to ${c.lawsCount}, dense and ascending (${c.laws.length} declared)`);
  const plantedGap = ['LAW.SIGIL.1', 'LAW.SIGIL.3'];
  say(!(plantedGap.length === c.lawsCount && plantedGap.every((k, i) => k === `LAW.SIGIL.${i + 1}`)),
    'trip: a laws list with a gap is refused against the declared count');
  const m = measure(c);
  ...
  // the run on this leg
  const r = run('all', c);
  // The census: every shell form of tiers S, A and B has a trial by name,
  // every trial names a declared form, and the run at scope all executed
  // exactly that many plus the tier D forms shown under a rule. The first
  // version compared the run to TRIALS itself, which at scope all is the
  // same array, and the fifth companion pass measured two declared forms
  // never run behind a green.
  const shellForms = ['S', 'A', 'B'].flatMap((t) => c.tiers[t]).filter((n) => TOKENS[n] && TOKENS[n].context === 'shell');
  const untried = shellForms.filter((n) => !TRIALS.some((t) => t.form === n));
  const shownD = TRIALS.filter((t) => c.tiers.D.includes(t.form)).map((t) => t.form);
  const undeclaredTrials = TRIALS.map((t) => t.form).filter((n) => !shellForms.includes(n) && !shownD.includes(n));
  say(r.leg.bash !== '' && untried.length === 0 && undeclaredTrials.length === 0 && r.trials.length === shellForms.length + shownD.length,
    `the run knows its leg (${r.leg.os}, bash ${r.leg.bash}) and the census holds: ${shellForms.length} shell forms of tiers S, A and B each have a trial, ${shownD.length} tier D form shown under its rule, ${r.trials.length} trials run${untried.length ? `; untried ${untried.join(',')}` : ''}${undeclaredTrials.length ? `; undeclared ${undeclaredTrials.join(',')}` : ''}`);
  ...
  // The older expectation: bash 3.2 answers 127 where bash 5 answers 1 to a
  // subshell whose expansion failed, and the first hosted run of 9.0.0 failed
  // its macOS leg on that one trial.
  const eiu = TRIALS.find((t) => t.form === 'error-if-unset');
  const held = r.trials.find((t) => t.form === 'error-if-unset');
  say(expectation(eiu, '3.2').expect === '127' && /^bash below 4.0 answers 127, this leg has 3.2$/.test(expectation(eiu, '3.2').note) && expectation(eiu, '4.0').expect === '1' && expectation(eiu, '5.3').expect === '1' && expectation(eiu, '5.3').note === '' && held.result === 'pass',
    `error-if-unset is held to 127 below bash 4.0 and to 1 from 4.0 on, the note naming the leg; this leg (bash ${r.leg.bash}) was held to ${held.expect} and passed`);
...
if (isMain) {
  ...
  if (verb === 'run') {
    ...
    for (const t of r.trials) console.log(`  ${t.result.padEnd(11)} ${t.tier} ${t.form.padEnd(26)} expect ${t.expect.padEnd(14)} actual ${JSON.stringify(t.actual)}${t.needs ? `  ${t.needs}` : ''}${t.note ? `  ${t.note}` : ''}`);
