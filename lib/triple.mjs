...
export function sigilFor(key, family = '') {
  const sigils = JSON.parse(readFileSync(join(ROOT, 'dtd', 'sigils.json'), 'utf8'));
  if (sigils[key]) return sigils[key];
  // Skills share the one Skills-family sigil declared in checker/readme-index.mjs
  // rather than minting twenty-three; the registry stays one sigil per command.
  if (family === 'skills') return '🎓';
  throw new Error(`triple: no sigil for ${key}; the emitter refuses an unsigned artifact`);
}
...
export function emit(src, outDir) {
  ...
  const family = src.split(/[\\/]/).includes('skills') ? 'skills' : '';
  const sigil = sigilFor(key, family);
