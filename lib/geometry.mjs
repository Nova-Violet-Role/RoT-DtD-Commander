...
//   surveyRoots(roots)           one survey per root, each printed and optionally written (GEO-F3)
//   plate(root, surveyRecord)    the plate alone from a survey record, written to GEOM.dir of the calling root, never into the surveyed tree (GEO-F4)
...
//
//   node lib/geometry.mjs survey [roots...] [--verbs=n,n] [--write] | plate <survey.json> [--write]
//                         | plan <survey.json> [--write] | plan --check <survey.json> <plan.json>
...
const IMPORT = /(?:import\s[^'"]*from\s*|import\s*\(\s*|require\(\s*)['"](\.{1,2}\/[^'"]+\.(?:mjs|js|cjs|mts|cts))['"]/g;
...
// ---------- widened shapes (GEO-F2) ----------
// The suite's own bin/*.mjs and lib/*.mjs come first, so a suite-shaped tree
// measures exactly what it always measured; anything else falls through to
// the manifest and then to generic mains, and only a tree with none of the
// three leaves the rung honestly unmeasured.
const SOURCE_EXT = /\.(mjs|js|cjs|mts|cts)$/;
function entryFiles(files, root) {
  const suite = files.filter((f) => /^bin\/[^/]+\.mjs$/.test(f.rel)).map((f) => f.rel);
  if (suite.length) return suite;
  try {
    const pkg = JSON.parse(readFileSync(join(root, 'package.json'), 'utf8'));
    const names = [...(pkg.bin && typeof pkg.bin === 'object' ? Object.values(pkg.bin) : [pkg.bin]), pkg.main]
      .filter((x) => typeof x === 'string').map((n) => String(n).replace(/^\.\//, ''));
    const hit = names.filter((n) => files.some((f) => f.rel === n));
    if (hit.length) return hit;
  } catch { /* no manifest */ }
  return files.filter((f) => /(^|\/)(main|index|app|cli)\.(mjs|js|cjs|mts|cts)$/.test(f.rel) && f.depth <= 1).map((f) => f.rel);
}
function libModules(files) {
  const suite = files.filter((f) => /^lib\/[^/]+\.mjs$/.test(f.rel)).map((f) => f.rel);
  if (suite.length) return suite;
  return files.filter((f) => /^src\//.test(f.rel) && SOURCE_EXT.test(f.rel)).map((f) => f.rel);
}
const EXPORT = /\bexport\s+(default\s+)?(const|let|var|function|class|\{|\*)/g;
...
const INSTRUMENTS = {
  ...
  2: (root, files) => {
    const entries = entryFiles(files, root);
  ...
  3: (root, files) => {
    const libs = libModules(files);
    if (!libs.length) return null;
    const fanIn = Object.fromEntries(libs.map((l) => [l, 0]));
    for (const f of files) if (SOURCE_EXT.test(f.rel)) for (const i of imports(root, f.rel)) if (i in fanIn && i !== f.rel) fanIn[i]++;
  ...
  6: (root, files) => {
    const mods = files.filter((f) => SOURCE_EXT.test(f.rel)).map((f) => f.rel);
    const have = new Set(mods);
    const graph = new Map();
    for (const m of mods) {
      const outs = imports(root, m).filter((i) => have.has(i) && i !== m);
      if (outs.length) graph.set(m, outs);
    }
    ...
    const visit = (n, seen) => {
      ...
      for (const o of graph.get(n) || []) {
        ...
        d = Math.max(d, 1 + visit(o, new Set([...seen, o])));
      }
      return d;
    };
    let best = { edges: 0, from: '' };
    for (const n of graph.keys()) {
      const d = visit(n, new Set([n]));
      if (d > best.edges) best = { edges: d, from: n };
    }
    if (!best.edges) return null;
    return { value: best.edges, unit: 'edges of the longest import chain', detail: { from: best.from } };
  },
  7: (root, files) => {
    const entries = entryFiles(files, root);
    if (!entries.length) return null;
    ...
    const dist = (a, b) => {
      const pa = a.split('/').slice(0, -1), pb = b.split('/').slice(0, -1);
      let i = 0;
    ...
    for (const e of entries) {
      ...
      let frontier = imports(root, e).filter((t) => have.has(t));
      while (frontier.length) {
        ...
        for (const t of frontier) {
          ...
          const d = dist(e, t);
          if (!far || d > far.hops) far = { rel: t, hops: d };
          for (const u of imports(root, t)) if (have.has(u) && !done.has(u)) next.push(u);
        }
        frontier = next;
      }
    }
    if (!far) return null;
    return { value: far.hops, unit: 'directory hops from entry to farthest use', detail: { farthest: far.rel } };
  },
  8: (root, files) => {
    const mods = [...new Set([...libModules(files), ...entryFiles(files, root)])];
    if (!mods.length) return null;
    let exp = 0;
    for (const m of mods) {
      try { exp += (readFileSync(join(root, m), 'utf8').match(EXPORT) || []).length; }
      catch { /* an unreadable module exports nothing countable */ }
    }
    return { value: Number((exp / mods.length).toFixed(2)), unit: 'exports per module', detail: { modules: mods.length, exports: exp } };
  },
  9: (root, files) => {
    let p = join(root, 'README.md');
    if (!existsSync(p)) {
      const alt = files.map((f) => f.rel).filter((r) => /^README\.[^.]+$/.test(r) || /^docs\/(index|readme)\.[^.]+$/.test(r)).sort();
      if (!alt.length) return null;
      p = join(root, alt[0]);
    }
    const lines = readFileSync(p, 'utf8').split('\n');
    let i = lines.findIndex((l) => /\/[a-z0-9-]+-dtd\b/.test(l));
    if (i < 0) i = lines.findIndex((l) => /^```/.test(l));
    return { value: i < 0 ? lines.length : i + 1, unit: 'lines before the first invocation or example', detail: { found: i >= 0 } };
  },
  11: (root) => {
    const g = spawnSync('git', ['-C', root, 'diff', '--numstat'], { encoding: 'utf8', timeout: 60000 });
    if (g.status !== 0) return null;
    const rows = String(g.stdout || '').split('\n').filter(Boolean);
    let ins = 0, del = 0;
    for (const r of rows) {
      const m = /^(\d+)\s+(\d+)\s+/.exec(r);
      if (m) { ins += Number(m[1]); del += Number(m[2]); }
    }
    if (!rows.length) return { value: 0, unit: 'files differ from HEAD', detail: { insertions: 0, deletions: 0 } };
    return { value: rows.length, unit: 'files differ from HEAD', detail: { insertions: ins, deletions: del } };
  },
  12: (root, files) => {
  ...
  14: (root, files) => {
    ...
    const schemas = files.filter((f) => /\.(xsd|rng|rnc|sch)$/.test(f.rel));
    if (!dtds.length && !schemas.length) return null;
    let n = 0;
    for (const f of dtds) n += (readFileSync(join(root, f.rel), 'utf8').match(/<!(ELEMENT|ENTITY|ATTLIST|NOTATION)\b/g) || []).length;
    const byKind = {};
    for (const f of schemas) {
      const t = readFileSync(join(root, f.rel), 'utf8');
      const k = f.ext === '.xsd' ? (t.match(/<(xs:|xsd:)?element\b/g) || []).length
        : f.ext === '.rng' ? (t.match(/<define\b/g) || []).length
        : f.ext === '.rnc' ? (t.match(/^\s*[a-zA-Z][\w.-]*\s*=/gm) || []).length
        : (t.match(/<(s:)?assert\b/g) || []).length;
      byKind[f.ext] = (byKind[f.ext] || 0) + k;
      n += k;
    }
    return { value: n, unit: 'declarations', detail: { subsets: dtds.length, schemas: schemas.length, byKind } };
  },
  15: (root, files) => {
    const byExt = {};
...
export function survey(root = ROOT, { c = contract(), verbs = null, fc = figureContract() } = {}) {
  ...
  for (const n of want) {
    const v = verbOf(n, c);
    if (!v) continue;
    if (n === 10 || n === 13 || n === 17) continue; // computed from the others, below
  ...
  if (want.includes(13)) {
    const at = (n) => measures.find((m) => m.verb === n);
    if (at(4) && at(5) && at(7)) {
      const ds = [Number(at(4).value), Number(at(5).value), Number(at(7).value)];
      measures.push({ verb: 13, name: 'trilateration', value: Number((ds.reduce((s, v) => s + v, 0) / ds.length).toFixed(2)), unit: 'mean of three measured distances', instrument: 'lib/geometry.mjs instrument 13', seconds: '0.000', confidence: 'measured', detail: { hops: ds[0], levels: ds[1], dataHops: ds[2] } });
    } else {
      unmeasured.push({ verb: 13, name: 'trilateration', why: 'needs measures 4, 5 and 7 in scope' });
    }
  }
...
// ---------- one survey per root (GEO-F3) ----------
// A five-dir scoped intake is one run here, not five runs plus hand
// aggregation: each root is surveyed separately with the same contract,
// and --write writes each under its own GEOM.dir.
export function surveyRoots(roots, { verbs = null, write = false, c = contract() } = {}) {
  return roots.map((r) => {
    const root = resolve(r);
    const s = survey(root, verbs ? { verbs, c } : { c });
    return { root, survey: s, wrote: write ? writeSurvey(root, s, { c }) : [] };
  });
}

// ---------- the plate alone (GEO-F4) ----------
// Plates used to arrive only beside a survey written per surveyed root,
// which would plant artifact directories inside src and docs. plate()
// re-renders the figure from a survey record and writes the svg plates to
// GEOM.dir of the CALLING root, never into the surveyed tree.
export function plate(root, s, { c = contract(), date = null, write = false } = {}) {
  const figureXml = drawFigure(s.measures, { grid: '80x12', mark: 'measured', title: `plate of ${basename(s.target || root)}` });
  const fig = parseFigure(figureXml);
  const v = validateFigure(fig, 'artifact');
  if (!v.ok) throw new Error('the plate figure is refused: ' + v.refused.join('; '));
  const cells = renderCells(fig), svg = renderSvg(fig, 'light'), svgDark = renderSvg(fig, 'dark');
  const agree = checkFigure(fig, cells, svg);
  if (!agree.ok) throw new Error('the plate figure disagrees with itself: ' + agree.findings.join('; '));
  const out = { figure: { xml: figureXml, cells, svg, svgDark }, paths: [] };
  if (write) {
    const dir = join(root, c.dir);
    mkdirSync(dir, { recursive: true });
    const d = s.date || stamp(date);
    writeFileSync(join(dir, `${d}-plate.svg`), svg, 'utf8');
    writeFileSync(join(dir, `${d}-plate-dark.svg`), svgDark, 'utf8');
    out.paths = [`${c.dir}/${d}-plate.svg`, `${c.dir}/${d}-plate-dark.svg`].map((p) => rel(root, join(root, p)));
  }
  return out;
}
...
export function controls(io = console) {
  ...
  say(c.instrumented.every((n) => INSTRUMENTS[n] || [10, 13, 17].includes(n) || n >= 18) && c.bands.surveyor.filter((n) => c.instrumented.includes(n)).length === 17,
  ...
  try {
    ...
    say(m(6).value === 2 && /cli\.mjs$/.test(m(6).detail.from), `altimetry walks the longest import chain at ${m(6).value} edges from ${m(6).detail.from}`);
    say(m(7).value === 2 && m(7).detail.farthest === 'lib/a.mjs', `odometry measures the farthest data move at ${m(7).value} directory hops: ${m(7).detail.farthest}`);
    say(m(8).value === 0.75 && m(8).detail.modules === 4, `tachymetry counts ${m(8).detail.exports} exports over ${m(8).detail.modules} modules: ${m(8).value} per module`);
    say(m(13).value === 2.33, `trilateration fixes position from three distances at ${m(13).value} (hops ${m(13).detail.hops}, levels ${m(13).detail.levels}, data hops ${m(13).detail.dataHops})`);
    say(s.unmeasured.map((u) => u.verb).join() === '11,14,16', `no dark rung remains: unmeasured is exactly what the tree gives nothing to read: ${s.unmeasured.map((u) => u.verb).join(', ')} (LAW.GEOM.2)`);
    // GEO-F3/F4: a scope filter, several roots in one run, a plate-only writer
    const scoped = survey(tmp, { c, verbs: [1, 5] });
    say(scoped.measures.map((mm) => mm.verb).join() === '1,5' && scoped.unmeasured.length === 0, 'trip: --verbs scopes the survey to the named rungs and nothing else');
    const pair = surveyRoots([tmp, tmp]);
    say(pair.length === 2 && pair[0].survey.read === pair[1].survey.read && pair.every((p) => p.wrote.length === 0), 'two roots in one run are surveyed separately with no hand aggregation and nothing written');
    const plated = plate(join(tmp, 'calling'), { ...s, target: tmp }, { write: true, c });
    say(plated.paths.length === 2 && plated.paths.every((p) => existsSync(join(tmp, 'calling', p))) && !existsSync(join(tmp, c.dir)), `the plate writer aims at GEOM.dir of the calling root only: ${plated.paths.join(', ')}`);
...
if (isMain) {
  ...
  if (verb === 'survey') {
    const write = args.includes('--write');
    const verbsArg = (args.find((a) => a.startsWith('--verbs=')) || '').slice('--verbs='.length);
    const verbs = verbsArg ? verbsArg.split(',').map(Number).filter((n) => n >= 1 && n <= 108) : null;
    if (verbsArg && !verbs.length) { console.error('survey: --verbs takes 1..108 comma-separated'); process.exit(2); }
    const roots = args.slice(1).filter((a) => !a.startsWith('--'));
    const list = roots.length ? roots : [process.cwd()];
    for (const { root, survey: s, wrote } of surveyRoots(list, verbs ? { verbs, write } : { write })) {
      ...
      if (write) for (const p of wrote) console.log(`  wrote ${p}`);
    }
    process.exit(0);
  }
  if (verb === 'plate' && args[1] && !args[1].startsWith('--')) {
    const s = JSON.parse(readFileSync(args[1], 'utf8'));
    const root = process.cwd();
    const r = plate(root, s, { write: args.includes('--write') });
    console.log(r.figure.cells);
    if (args.includes('--write')) for (const p of r.paths) console.log(`  wrote ${p} (calling root ${root}, surveyed tree untouched)`);
    process.exit(0);
  }
