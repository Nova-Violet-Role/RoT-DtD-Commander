...
export function packageLines(pkgText) {
  ...
  return Object.values(j.scripts || {}).join('\n');
}
...
export function matrix(text, c = contract()) {
  ...
  // install-roundtrip, read from the text this function was handed.
  const rel = (text.split(/^  release:\s*$/m)[1] || '');
...
export function controls(io = console) {
  ...
  const noNeeds = wfText.replace(/\n    needs: \[gate, install-roundtrip\]/, '');
  const m3 = matrix(noNeeds, c);
  say(noNeeds !== wfText && m3.findings.some((x) => /release job does not need/.test(x)), `trip: a release job with its needs removed is refused: ${m3.findings.find((x) => /release/.test(x)) || 'nothing'} (LAW.XOS.6)`);
  const pk = sweepText(packageLines('{"scripts":{"gate":"timeout 5 node a.mjs && node b.mjs","ok":"node lib/ceiling.mjs 5 node a.mjs"}}'), 'package.json', c);
  say(pk.length === 1 && pk[0].form === 'timeout' && pk[0].file === 'package.json', `trip: a form inside a package.json script is refused: ${pk.map((x) => `${x.form}@${x.line}`).join(' ') || 'nothing'}`);
