...
description: Audits the shared DTD subsets (every dtd/cc-*.dtd, twenty-two of them, and the root grammars beside them) against every *-dtd command, skill and agent in a repository or an installed .claude tree, in both directions. Invoke after editing a shared subset, adding a *-dtd artifact, or before an install, to find declarations nothing uses and files that drift from the contract.
...
<!DOCTYPE contract_audit [
  ...
  <!ENTITY LAW.CONTRACT.2 "Direction two: every DOCTYPE-bearing file in the corpus resolves and passes rules C1 to C16; a failing file is listed with its first failing rule.">
...
3. For every corpus file run `node <repo>/bin/rot-dtd-commander.mjs check <file>` with a 60 second ceiling when the repository is available, else apply C1 to C16 by hand, and write one `checked` element with pass or fail and the first failing rule.
