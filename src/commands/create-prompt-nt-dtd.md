...
<!DOCTYPE prompt_forge [
  ...
  <!ATTLIST prompt_forge schematic (nt) #FIXED "nt">
  ...
  <!ENTITY LAW.PROMPT.1 "The schematic of this command is nt, fixed in its DOCTYPE; the file it writes is a NestedText document and every syntax in it comes from a SCHEMA.nt.* entity (LAW.SCHEMA.1).">
  <!ENTITY LAW.PROMPT.2 "The sections are those of SCHEMA.prompt.sections, rendered in that order; the argument words are embedded through SCHEMA.nt.reference and SCHEMA.nt.literal in the class the intake chose (LAW.SCHEMA.2, LAW.SCHEMA.3).">
  ...
  <!ENTITY LAW.PROMPT.4 "The file takes the extension SCHEMA.ext.nt, carries the chosen SPDX identifier where its form allows a comment, and passes every cc-form guard of its kind before it is reported (LAW.SCHEMA.4).">
...
<objective>
Create a prompt for <quoted trust="cdata" source="user-args">$ARGUMENTS</quoted> (or ask what it is for), written in a NestedText document.

The schematic is pinned: each section a key with a multiline string, an angle bracket per line. What a literal is, what expands, how a value is referenced, defined, escaped, commented, included or made conditional, is read from the SCHEMA.nt.* table of cc-schematic.dtd, the table cut from the argument-variant references: the quoted heredoc is the CDATA section is the strip block scalar is the NestedText multiline string, and the argument string is always the quoted whole. The sections are SCHEMA.prompt.sections. The file is guarded by cc-form before it is reported and proven by a planted out-of-table syntax that the proof refuses.
</objective>
...
4. Render the `sections`: one `section` per name of SCHEMA.prompt.sections, in order, each with its text; render the `schemas`: one `semantic` per schema chosen, its `part` elements from the SEMANTIC entity of that schema with occurs one, optional or many, each rendered by SEMANTIC.nt.part, SEMANTIC.nt.many and SEMANTIC.nt.label (LAW.PROMPT.6); render the `embedding`: the reference syntax SCHEMA.nt.reference, the literal syntax SCHEMA.nt.literal, and the cc-args class chosen for the argument words.
