...
<!DOCTYPE mcp_forge [
  ...
  <!ENTITY LAW.MCP.1 "Round one always runs before anything is written, even when the argument reads complete; --no-gate alone skips the rounds, and then every answer is an assumption_made (LAW.ASK.10).">
  <!ENTITY LAW.MCP.2 "The create-mcp-servers-dtd skill is invoked once, through the Skill tool, with the purpose, then ARG.end, then the answers as known slots (name=, and one word per question answered); the skill writes a Model Context Protocol server with its tool schemas and installation and this command reads it back; nothing is written before that invocation.">
  <!ENTITY LAW.MCP.3 "Every file written is re-read and rendered with its path and bytes, passes the cc-form guards of its kind, and is headed by the license expression where its format allows a comment, headed no otherwise (LAW.LICENSE.1, LAW.LICENSE.2, LAW.FORM.2).">
  <!ENTITY LAW.MCP.4 "The audit runs here, in the foreground, under a 60 second ceiling with stdin closed: the server built or imported under the ceiling (M1), started with an initialize and a tools/list message fed from a file, never a terminal (M2), every tool schema checked for a description and typed inputs (M3), and each checkpoint of the validation reference of the skill run (M4); one rule element per code with pass, fail or skipped; a fail is a failed answer; no subagent is summoned for it.">
  <!ENTITY LAW.MCP.5 "The proof plants one fault in a scratch copy (a tool schema stripped of its description) and shows the audit refuse it; a proof that did not trip stops the command before the report.">
  <!ENTITY LAW.MCP.6 "The emoji chosen heads every heading of the artifact's answers (LAW.CORE.6); when the artifact lands in this repository it is registered in dtd/sigils.json, and a glyph already bound there is refused and the question asked again.">
  <!ENTITY ASK.MCP.1 "Name|What is the server called?|A kebab-case name from the argument|The system it fronts, -mcp suffixed|Typed under Other|Undecided, ask again after the language">
  <!ENTITY ASK.MCP.2 "Language|Which implementation?|TypeScript with the official SDK|Python with the official SDK|Both, TypeScript first|Typed under Other">
  <!ENTITY ASK.MCP.3 "Transport|Which transport?|stdio|Streamable HTTP|Both|Typed under Other">
  <!ENTITY ASK.MCP.4 "Tools|How many tools?|One|A few, named under Other|A large API through the large-api pattern of the skill|Typed under Other">
  <!ENTITY ASK.MCP.5 "Surface|Beyond tools?|Tools only|Tools and resources|Tools, resources and prompts|Typed under Other">
  <!ENTITY ASK.MCP.6 "Emoji|Which emoji heads its headings?|The family default, 🔌|One typed under Other|None|Undecided, the first free glyph of the roster">
  <!ENTITY ASK.MCP.7 "Voice|Which voice profile?|Original, prepared, factual, the text_desc defaults|Paraphrase of a named source, cited|Spontaneous|Typed under Other">
  <!ENTITY ASK.MCP.8 "Record|Where does this run record?|artifacts under this command's name, command-generated filename|Nowhere|Typed under Other|Undecided">
  <!ENTITY ASK.MCP.9 "Audit|Which audit runs after the write, here in the foreground?|The server rules M1 to M4|The server rules M1 to M4 only|None, which this command refuses|Typed under Other">
  <!ENTITY ASK.MCP.10 "Proof|How is it proven?|Plant one fault in a scratch copy and show the audit refuse it|Read back only|None, which this command refuses|Typed under Other">
]>
...
<objective>
Create an MCP server for <quoted trust="cdata" source="user-args">$ARGUMENTS</quoted> (or ask what it is for): a Model Context Protocol server with its tool schemas and installation.

This command is the door in front of the create-mcp-servers-dtd skill. It asks the twelve questions that shape the artifact, the license from the curated list, the emoji and the form, then hands every answer to the skill as known slots so the skill asks nothing twice, reads back what the skill wrote, guards it, audits it in the foreground with one rule per code, and proves the audit by a planted fault. The audit that used to be a dispatch to a subagent lives here now.
</objective>
...
- `invocation`: **🔌 Invocation**, the one Skill call to create-mcp-servers-dtd with its argument
...
- `audit`: **🔌 Audit**, one line per rule (M1 to M4) with pass, fail or skipped
- `proof`: **🔌 Proof**, the planted fault, the rule that refused it, tripped yes or no
- `assumption_made`: **🔌 Assumptions Made**, every ASK.MCP.* question not asked, with the first option taken
</grammar_map>
...
Skill create-mcp-servers-dtd with "[purpose] -- name=[name] [one word per answer]"
...
- create-mcp-servers-dtd was invoked once with the known slots and asked none of them again
