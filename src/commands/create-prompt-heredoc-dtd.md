...
4. Render the `sections`: one `section` per name of SCHEMA.prompt.sections, in order, each with its text; render the `embedding`: the reference syntax SCHEMA.heredoc.reference, the literal syntax SCHEMA.heredoc.literal, and the cc-args class chosen for the argument words.
