...
6. Diff: `node lib/ceiling.mjs 60 node lib/scratch.mjs diff <topic>` for the files with their counts, and `git diff <base>...<branch>` for the hunks; write one `finding` per hunk with file, hunk, verdict kept, changed or dropped, severity and confidence (LAW.DS.3); a changed or dropped hunk is edited in the worktree and committed before the next phase.
