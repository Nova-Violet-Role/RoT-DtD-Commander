...
- `intake`: **🌟 Intake**, the known and gap slots, then each round as n of 8 with its block number, its questions and its answers, and the gate choice; the gate offers GATE.save as its fifth choice (LAW.CACHE.1), and on save the `cache` element names the file written and read back whole (LAW.CACHE.2), or on the next call the file resumed from (LAW.CACHE.3)
