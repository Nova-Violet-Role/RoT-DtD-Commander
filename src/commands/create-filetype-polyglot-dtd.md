...
<!DOCTYPE filetype_forge [
  ...
  <!ATTLIST filetype_forge schematic (polyglot) #FIXED "polyglot">
  <!ATTLIST filetype name NMTOKEN #REQUIRED ext NMTOKEN #REQUIRED notation NMTOKEN #REQUIRED kind (polyglot) #FIXED "polyglot">
  ...
  <!ENTITY LAW.FTPOLYGLOT.1 "Round one always runs before anything is written, even when the argument reads complete; --no-gate alone skips the rounds, and then every answer is an assumption_made (LAW.ASK.10).">
  <!ENTITY LAW.FTPOLYGLOT.2 "The file type is declared, not described: a name, an extension, a NOTATION from cc-form or typed with its MIME type, and the kind polyglot pinned by this command; the schematic is polyglot and never asked.">
  <!ENTITY LAW.FTPOLYGLOT.3 "Every dollar-token variant marked is embedded in the exemplar the way this schematic embeds a reference or a literal (SCHEMA.polyglot.reference, SCHEMA.polyglot.literal), so it never expands: the exemplar re-read carries every marked token verbatim, counted with a fixed-string search, never a regex (LAW.SCHEMA.4).">
  <!ENTITY LAW.FTPOLYGLOT.4 "Two files are written under FTPOLYGLOT.dir, UTF-8 LF without BOM: the exemplar, the name followed by the extension, carrying the parts of every schema chosen from node lib/schematic.mjs render and the marked tokens embedded; and the declaration, the name followed by .notation.dtd, one NOTATION line naming the parser, the schematic and the tokens, and one entity per marked token; both re-read and rendered with their bytes.">
  <!ENTITY LAW.FTPOLYGLOT.5 "Both files pass the cc-form guards of their kind, and the exemplar is headed by the license where the form allows a comment (LAW.LICENSE.2, LAW.FORM.2).">
  <!ENTITY LAW.FTPOLYGLOT.6 "The proof plants one marked token in an expanding position in a scratch copy of the exemplar (an unquoted here-document, a YAML flow value, a parsed-text element, a callout title with a bare dollar) and shows the guard or the read-back refuse it; a proof that did not trip stops the command before the report.">
  <!ENTITY LAW.FTPOLYGLOT.7 "The four variants appear in this command: the extension, the notation and the purpose are selects, the schemas and the forms checks, the variants a mark and the embedding an elaborate, each option elaborated before the ask (LAW.ASK.13).">
  <!ENTITY ASK.FTPOLYGLOT.1 "Name|What is the file type called?|A kebab-case name from the argument|The name of what it carries, as a noun|Typed under Other|Undecided, ask again after the purpose">
  <!ENTITY ASK.FTPOLYGLOT.2 "Extension|Which extension?|md, the schematic's own|A second extension typed under Other, with md kept|The name itself as the extension|Typed under Other">
  <!ENTITY ASK.FTPOLYGLOT.3 "Notation|Which NOTATION names its parser?|markdown, the schematic's own from cc-form|A NOTATION typed under Other, with its MIME type|The cc-form NOTATION of the outermost layer|Typed under Other">
  <!ENTITY ASK.FTPOLYGLOT.4 "Purpose|What does a file of this type carry?|The one thing named in the argument|A record series, one file per run|A configuration a command reads|Typed under Other">
  <!ENTITY ASK.FTPOLYGLOT.5 "Variants|Which dollar-token variants does the type embed? Each is elaborated first; mark the ones that apply.|The whole argument string|The first positional word|Every positional word quoted whole|A positional word with a default, and the flags">
  <!ENTITY ASK.FTPOLYGLOT.6 "Embedding|How is a marked token embedded so it never expands? Each way is elaborated first.|The way this schematic embeds a reference (SCHEMA.polyglot.reference)|The way it embeds a literal (SCHEMA.polyglot.literal)|Both, the reference inside a literal|Typed under Other">
  <!ENTITY ASK.FTPOLYGLOT.7 "Record|Where does this run record?|artifacts under this command's name, command-generated filename|Nowhere|Typed under Other|Undecided">
  <!ENTITY ASK.FTPOLYGLOT.8 "Proof|How is it proven?|Plant one token in an expanding position in a scratch copy and show the guard or the read-back refuse it|Read back only|None, which this command refuses|Typed under Other">
  <!ENTITY FTPOLYGLOT.dir "filetypes">
  <!ENTITY FTPOLYGLOT.tokens "the whole argument string; the first positional word; every positional word quoted whole; a positional word with a default; the flags">
]>
...
<objective>
Create a free file type pinned to the polyglot schematic for <quoted trust="cdata" source="user-args">$ARGUMENTS</quoted> (or ask what it carries): a polyglot of more than one parser, with the semantic schemas and the dollar-token variants the user marks.
...
[name].[ext]; notation [markdown or typed]; kind polyglot; schematic polyglot
...
- [token]: embedded [yes|no], as [SCHEMA.polyglot.reference | SCHEMA.polyglot.literal | both]
...
`filetypes/[name].notation.dtd` ([bytes] B): NOTATION [name] SYSTEM "[mime]; polyglot; variants [tokens]"
