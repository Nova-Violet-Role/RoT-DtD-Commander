...
<declared_grammar>
<grammar_map>
Render the `investigation` root declared in the DOCTYPE as the markdown below. One declared element per heading, in declared order; a required element with nothing to say still appears, with one line saying so.
- `symptom`: what was observed
- `evidence`: each piece with id and kind
- `hypothesis`: each with `prediction`, `test`, `verdict`
- `root_cause`: the confirmed hypothesis by IDREF
- `fix`: the change made
- `verification`: the run that shows the symptom gone, exit code read directly
</grammar_map>
