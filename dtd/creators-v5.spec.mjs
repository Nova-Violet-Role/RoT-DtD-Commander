...
const AUDIT = {
  check: {
    rules: 'the contract rules C1 to C16',
    text: (c) => `node bin/rot-dtd-commander.mjs check on every -dtd file written, one rule per code C1 to C16, then the style areas the ${c.auditor} agent file lists, read as data and checked one by one`,
    fault: 'an element declared and never named, or a law numbered out of sequence',
    codes: 'C1 to C16 and the style areas',
  },
  hook: {
...
This command is the door in front of the ${c.skill} skill. It asks the twelve questions that shape the artifact, the license from the curated list, the emoji and the form, then hands every answer to the skill as known slots so the skill asks nothing twice, reads back what the skill wrote, guards it, audits it in the foreground with one rule per code, and proves the audit by a planted fault. The audit that used to be a dispatch to a subagent lives here now.`,
    process: [
      ...
      `Round 1 of 3: ask ASK.${P}.1 to ASK.${P}.4 (select) as one AskUserQuestion call, four options each plus Other; render the round with the variant beside each question (LAW.ASK.13).`,
      `Present the gate; on more, round 2 of 3 with ASK.${P}.5 (select), ASK.${P}.6 (select), ASK.LICENSE.1 (mark: each license elaborated, the marked ones joined) and ASK.FORM.1 (check); on more again, round 3 of 3 with ASK.${P}.7 (elaborate: each voice profile elaborated before the ask), ASK.${P}.8 (select), ASK.${P}.9 (select) and ASK.${P}.10 (select); on add or impactful, take the answer and present the gate again; on start, proceed with every unasked question at its first option, listed under Assumptions Made.`,
...
function audit(a) {
  ...
  return [a.key, {
    ...
    description: `DTD-native: audit ${a.what} here, in the foreground: the contract rules C1 to C16 through the checker under a ceiling, then the style areas of ${a.auditor} read from its agent file as data and checked one by one; findings with file and line, one verdict; no subagent is summoned`,
...
The audit that used to be a dispatch to a subagent runs in the foreground now: the checker under a ceiling, the areas AUDIT.areas checked one by one, every finding with file and line, one verdict. The agent file stays for a hand summons; this command never summons it.`,
    process: [
      ...
      'Run AUDIT.checker on the target in the foreground, under AUDIT.ceiling seconds with stdin closed, the exit read directly; render the `contract` with one `rule` per code C1 to C16, result pass, fail or skipped, with the checker\'s line (LAW.AUD.2).',
    ...
    map: {
      ...
      contract: `**${s} Contract**, one line per rule C1 to C16 with pass, fail or skipped`,
