...
function promptCreator(schematic, meta) {
  ...
  const q = meta
    ? {
        ...
        5: 'Voice|Which voice profile?|Original, prepared, factual, the text_desc defaults|Paraphrase of a named source, cited|Spontaneous, for brainstorming prompts|Typed under Other',
        6: 'Record|Where does a run record?|artifacts under the prompt name, command-generated filename|Nowhere|Typed under Other|Undecided',
        7: 'Proof|How is it proven?|Read back, guards run, sections, schema parts and forms in order, one out-of-table syntax planted and refused|Read back only|None, which this command refuses|Typed under Other',
        8: 'License|Which SPDX header heads the file?|AGPL-3.0-or-later OR EUPL-1.2, the repository license|MIT|Apache-2.0|Typed under Other',
      }
    : {
  ...
  return [key, {
    ...
    model: [
      `${root} (args, intake, sections, schemas, forms, embedding, file, guards, proof, assumption_made*)`,
...
The schematic is pinned: ${s.example}. What a literal is, what expands, how a value is referenced, defined, escaped, commented, included or made conditional, is read from the SCHEMA.${schematic}.* table of cc-schematic.dtd, the table cut from the argument-variant references: the quoted heredoc is the CDATA section is the strip block scalar is the NestedText multiline string, and the argument string is always the quoted whole. The sections are ${sectionsEnt}. The file is guarded by cc-form before it is reported and proven by a planted out-of-table syntax that the proof refuses.`,
    ...
    map: {
      ...
      forms: `**${sigil} Forms**, one line per form chosen with its kind, variant and expansion, or one line saying nt, the default`,
...
- [each probe not asked, first option taken]`,
  ...
  'brainstorm-meta-clear-section': {
    new: true, to: 'src/commands/brainstorm-meta-clear-section-dtd.md', root: 'clear_section', include: ['cc-args', 'cc-form', 'cc-schematic', 'cc-ask'],
    ...
    entities: {
      ...
      'ASK.CLEAR.5': 'Kind|Is the carried prompt a prompt or a meta-prompt?|A prompt|A meta-prompt, a prompt that writes prompts|Undecided, a prompt for now|Typed under Other',
    ...
    laws: {
      ...
      'CLEAR.6': 'The schematic, the semantic schemas, the kind and the forms of the bigger prompt are chosen here, before the clear, and rendered as a launch element with its schematic, its kind and the creator they select: SCHEMA.creator.prompt or SCHEMA.creator.meta, a hyphen, the schematic and -dtd; the handoff carries every choice as a known slot, so the creator asks none of them again (LAW.SCHEMA.10, LAW.ASK.1).',
    },
...
The shape is borrowed from the instruction channel of the RoT DTD GOAL trust contract: when a queue advances, the gate is not reporting a result but issuing an instruction, and that is tagged as its own element with a goal and a step so a reader can tell what happened from what to do next. Here the instruction is always the same two moves: run CLEAR.command, then open the next section with the launch line that names the handoff file and the creator that matches the schematic chosen, so the bigger prompt is written in that schematic, with those schemas and those forms, by a creator that already knows them.`,
    process: [
      ...
      'Render the `instruction` with goal and step: step one is CLEAR.command, step two is the launch line, the at-sign reference to the handoff file followed by a slash, the creator named in the launch and the purpose of the bigger prompt as its argument.',
    ],
    map: {
...
- [each unasked question, first option taken, or none]`,
  ...
  'create-plugin': {
    new: true, to: 'src/commands/create-plugin-dtd.md', root: 'plugin_creation', include: ['cc-args', 'cc-license', 'cc-ask'],
    ...
    model: [
      ...
      'instruction (#PCDATA)', 'proof (#PCDATA)',
    ],
    attlist: [
    ...
    entities: {
      ...
      'ASK.PLUGIN.5': 'License|Which SPDX license?|AGPL-3.0-or-later OR EUPL-1.2, the license of this repository|MIT|Apache-2.0|An identifier or a compound expression from LICENSE.list, typed under Other',
    ...
    laws: {
      ...
      'PLUGIN.4': 'The SPDX identifier is one of LICENSE.list or a compound expression joining its identifiers with LICENSE.join, a double or a triple; an identifier outside the list is refused with the list printed (LAW.LICENSE.1); the chosen expression heads every file whose format allows a comment (LAW.LICENSE.2), rendered as the license element.',
