...
for (const f of FAMILY) {
  spec[`${f.key}-codebase`] = {
    ...
    attlist: [],
    laws: {
      [`${f.prefix}.4`]: `The page the generator offers is the size the answering earned, resumed from the state record and held under the ceiling the walk's own size allows; a run that opens on a fresh target starts at AMP.page and a practised operator is offered more (LAW.AMP.11).`,
      [`${f.prefix}.5`]: `The intake runs five rounds of at most four questions, twenty in all, and the rounds grow: round one is drawn from the walk, and every later round is generated from what the previous answers opened, so a round that would ask nothing new ends the intake early rather than asking to fill its count (LAW.ASK.6, LAW.ASK.11).`,
      [`${f.prefix}.6`]: `A run is bounded and the sequence of runs is not: the state record makes every later invocation continue where this one stopped, a refusal returns only as a reopen once its runs are up or a file beneath it moved, and a possibility marked done never returns (LAW.AMP.6, LAW.AMP.12).`,
    },
...
metamorphosis, with AMP.ladder.count saying how many there are. \`${f.key}-codebase.dtd\`
pins this command's share of it: ${f.prefix}.band lists the verb numbers, ${f.prefix}.low and
${f.prefix}.high its ends, ${f.prefix}.next the verb above it, ${f.prefix}.what the kind of change it
...
Four guards hold wherever this command reads or writes: ${f.prefix}.guard.quoting, so a
target path carrying a space is one word and never two; ${f.prefix}.guard.literal, so a
dollar sign, a brace, a backtick or a heredoc marker inside a finding survives
into the study byte for byte; ${f.prefix}.guard.pcdata, so a possibility's text is escaped
into PCDATA and never wrapped in a CDATA section, because a fragment carrying
the close delimiter would end the section early; and ${f.prefix}.guard.pentity, so a
...
Local evidence first, and the two classes are never confused: a \`possibility\` of class gap is measured, naming the instrument and the path that show a declaration the target disagrees with; a possibility of class idea is reasoned or guessed, naming in adds the law, entity or file it would create. The finite half is ranked first; the unbounded half is marked as what it is (LAW.AMP.3, LAW.CORE.4).`,
    process: [
      ...
      'Drop every id the state record closed, then rank: gaps before ideas, then risk, then breadth. A refusal whose AMP.reopen.after runs have passed, or whose files moved since, returns as verdict reopen carrying refused_at, and is offered as the second offer it is (LAW.AMP.6, LAW.AMP.12).',
      'Set the page: resume the size from the state record, move it by what the last round earned, and hold it under the ceiling the walk\'s own size allows; print the size beside the unshown count (LAW.AMP.5, LAW.AMP.11).',
      ...
      'Write the state record back with this run\'s number, the generator offset, the verb it ended on, the release badge and every possibility with its verdict; then render `next_verb` with `${f.prefix}.next`, the verb above `${f.prefix}.high`, and the command that owns it, the band having run from `${f.prefix}.low` and the kind of change being `${f.prefix}.what` (LAW.AMP.4, LAW.AMP.6).',
    ],
    map: {
...
**Next Verb:** [n] [name] — run /[command]`,
    success: [
      ...
      `The band is the one the subset pins, the guards are the ones it names, and the page is the size the answering earned (LAW.${f.prefix}.1, LAW.${f.prefix}.2, LAW.${f.prefix}.3)`,
    ],
  };
}
